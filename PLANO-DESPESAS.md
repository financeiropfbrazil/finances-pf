...
|Simulação do anti-join jan–jun (20/07)|✅ reproduz a §6 da spec **exatamente** nas 6 competências (jan 49 · fev 67 · mar 58 · **abr 66** · mai 59 · jun 72 = 371 / R$ 1.475.637), rodada contra o MovEstq de hoje (5.139 docs, all-dates). ⚠️ **Estes números são RETRATO de 13/07, não previsão** — o Alvo cresce por lançamento retroativo (junho já foi de 72 → **83 admitidos**, +9 docs no Alvo). **Re-medir cada competência imediatamente antes do backfill** (armadilha 38)|
...
>
> **Ao retomar, o crítico de aceitação de abril (AJUSTADO 22/07 — armadilha 38):** o esperado **não** é mais o congelado `antijoin_admite = 66` (retrato de 13/07). **Re-rodar o censo/simulação do anti-join de abril imediatamente antes de reabrir** (contra o MovEstq do dia) e usar **esse** número como esperado — o Alvo cresce por lançamento retroativo (junho subiu de 72 → 83 assim). **Se o real divergir do re-medido, ordem de investigação:** (1) a divergência sobrevive à re-medição de hoje? (2) se sim, **PAGINAÇÃO/índice** antes da lógica de admissão (a simulação dos 66 rodou em SQL direto, sem paginação — D-018); (3) por último, a lógica de admissão. Com o D-016 no ar, o delta de abril fica **puramente positivo** e o critério cai de 3 para 2 parcelas.
>
> **Invariantes a conferir em qualquer retomada:** `valor_fora_controle` = **R$ 0,00** · total do Hub = **R$ 32.058.639,92** (era o valor ao fim de 20/07) · `movestq_index_linhas` ≥ **5.139** (22/07: **5.178** — só cresce; se CAIR, quebrou) · `select count(*) from desp_docfin_doc where payload_alvo->>'Projecao'='Sim'` = **0**.
...
**Status:** ✅ **AUDITORIA EXECUTADA pelo Pedro 20/07/2026 — 2 pontos confirmados.** Correções definidas, a aplicar **junto com o passo 2 do D-016**, antes do backfill. · **Origem:** achado do Pedro ao revisar a RPC do D-016 (20/07/2026). · **Parente direto do D-006.**

> **Resultado da auditoria (`erp-proxy`):**
>
> |local|linha|`.order()`?|correção|
> |-|-|-|-|
> |**`carregarIndiceMovEstq`** (índice do anti-join)|**236–237**|❌|`.order("chave_movestq", { ascending: true })` antes do `.range()`|
> |**limpeza de órfãos** (chaves no banco)|**949–953**|❌|`.order("chave_docfin", { ascending: true })` antes do `.range()`|
> |`despesas.ts` (MovEstq)|—|não pagina|sem risco|
>
> **Ordenar pela PK, não por coluna do `select`** (decisão do Pedro): coluna não-única empata e a instabilidade volta — foi a causa raiz do D-006 (`numero` = data em RDESP, lote na mesma data, boundaries de página caindo no empate).
>
> **📌 A LIÇÃO (registrada por pedido do Pedro) — anotar o risco não é o mesmo que fechá-lo.** O comentário no próprio código dizia que aquela paginação era **"o ponto mais perigoso do arquivo"** — e mesmo assim ela foi escrita **sem `ORDER BY`**. **Diagnóstico certo, implementação incompleta:** a paginação resolveu o **truncamento** (o risco que havia sido identificado) e deixou passar a **instabilidade** (o risco que não havia). O comentário criou a sensação de que o ponto estava tratado, e foi essa sensação que impediu a segunda pergunta. → **armadilha 37**.
>
> **Baseline do índice (medido 20/07):** `desp_realizado_doc` = **5.139 linhas, 5.139 chaves distintas, 6 páginas de 1.000**. **Sinal de saúde: `movestq_index_linhas` ≥ 5.139 e crescendo devagar. Se CAIR, alguma coisa quebrou** — e a consequência é dupla contagem silenciosa, não erro.
...
37. **Anotar o risco não é o mesmo que fechá-lo — e o comentário pode ser o que impede a segunda pergunta (adicionada 20/07/2026 — regra de método, irmã da 20).** Caso concreto (D-018): o comentário no `erp-proxy` dizia que aquela paginação era **"o ponto mais perigoso do arquivo"**, e ela foi escrita **sem `ORDER BY`**. O autor identificou o risco **certo** (truncamento — a lista não cabia numa resposta) e implementou a solução **certa para aquele risco** (paginar). O que ficou de fora foi um risco **diferente, criado pela própria solução**: paginação sem ordenação estável (armadilha 36). **Diagnóstico certo, implementação incompleta.** Pior: o comentário de alerta produziu a *sensação* de que o ponto estava tratado — quem lesse depois veria "risco conhecido e endereçado" e não faria a segunda pergunta. **Regra: ao encontrar um comentário que sinaliza perigo, tratá-lo como convite para auditar, nunca como atestado de que o problema foi resolvido.** E, ao mitigar um risco, perguntar explicitamente **quais riscos NOVOS a mitigação introduz** — paginar resolve truncamento e cria instabilidade de ordem; cache resolve latência e cria staleness; retry resolve falha transitória e cria duplicação. A armadilha 20 diz que dedução não substitui medição; esta diz que **sinalização não substitui correção**.

38. **O censo/§6 é RETRATO do dia em que foi medido, não previsão — re-medir a competência imediatamente antes de reabri-la (adicionada 22/07/2026).** A tabela §6 da `SPEC-D001-erp-proxy.md` (jan 49 · fev 67 · mar 58 · abr 66 · mai 59 · jun 72) foi tirada em **13/07** e mede o Alvo **daquele dia**, não do dia do backfill. **Prova em junho:** os admitidos subiram para **83** contra os **72** previstos, porque o Alvo **ganhou 9 documentos** entre o censo de 13/07 (**619 listados**) e hoje (**628**) — **lançamento retroativo, normal e não-anômalo**. As contas fecham: **360 anti-join + 47 legado + 221 FAT/sem chave = 628.** **A hipótese de truncamento de índice (armadilha 36) foi DESCARTADA por evidência de direção:** o índice MovEstq **cresceu** (5.122 → 5.178) e mesmo assim as admissões subiram — se fosse truncamento (linha faltando no índice → doc admitido por engano), o efeito andaria junto com o índice **encolhendo**, não crescendo. **Origem confirmada: o Alvo, não o índice.** **Consequência para o critério de aceitação do backfill:** o número congelado (`abr = 66`) **não é a previsão para o dia da reabertura**. Antes de reabrir CADA competência, **re-rodar o censo/simulação do anti-join daquela competência contra o MovEstq do dia** e usar **esse** número como esperado. **Divergência do número congelado não é automaticamente bug de paginação** (armadilha 36): pode ser crescimento legítimo do Alvo. Ordem de investigação: (1) re-medir a competência hoje — a divergência sobrevive? (2) se sim, paginação/índice; (3) por último, a lógica de admissão. Jan–abr devem variar **menos** que junho (competências mais antigas, menos lançamento retroativo esperado), mas o princípio vale para todas: **medir imediatamente antes, nunca confiar em número de uma semana atrás.** Irmã da armadilha 20 (dedução≠medição) no eixo do TEMPO — aqui a medição está certa, mas **envelhece**.

39. **A DHL tem DOIS padrões de numeração no espelho, e a regra `FRCIR` só cobre um (adicionada 11/08/2026 — achado de CAPTURA, levantado no levantamento do razão jan–abr).** O razão contábil escreve `FRCIR02367521`; o espelho (`desp_realizado_doc`) grava o **mesmo documento** como **`42367521`** — um `4` onde o razão tem o zero. Convive com o padrão já conhecido e documentado (`FRCIR02357967` → espelho `02357967`). Pior: o prefixo **não é estável** — um dos casos veio como **`2942405715`** (prefixo `294`, número de 10 dígitos), então não é "um dígito a mais". **Medição:** dos 11 documentos DHL do razão sem match por número, **7 estão no espelho** com prefixo divergente — valor batendo **ao centavo nos 7**, data 0–5 dias de diferença, **R$ 173.415,14**. Confirmado por dois métodos independentes (número truncado; e valor+nome+data sem olhar número nenhum), que convergiram linha a linha nos mesmos 7 documentos. **NÃO generalizar por "últimos 7 dígitos":** medido sobre o espelho inteiro, **24 grupos de últimos-7 colidem** entre `num_k` distintos, **4.031 documentos** do espelho e **831 das 1.179 linhas do razão (70%)** têm número normalizado com menos de 7 dígitos — a regra é inaplicável à maioria e criaria falso positivo onde hoje há acerto. **Regra: casamento por número truncado só com fornecedor fixado + valor idêntico como confirmação, nunca como regra global.**

40. **`desp_docfin_doc.chave_movestq` existe mas é 100% NULO — não serve de ponte entre as rotas (adicionada 11/08/2026 — achado de CAPTURA).** A coluna está lá (posição 19, `bigint`) e o nome promete a ligação DocFin→MovEstq, mas **0 de 611 documentos de jan–abr/2026 a têm preenchida**. Quem tentar deduplicar as duas rotas por ela obtém zero pares e conclui, erradamente, que não há sobreposição — quando **138 números normalizados aparecem nas duas fontes** no mesmo período. Parente direta da armadilha 14 (`chave_movestq` em `contas_pagar` é ~sempre null): **a mesma coluna promete a mesma ponte em três tabelas diferentes e não a entrega em nenhuma.** Cruzamento entre rotas continua sendo por identidade (espécie+entidade+número, armadilha 18) ou por valor+nome+data — nunca por essa chave.

41. **O Alvo grava BRAZIL e a contabilidade grava BRASIL — e um prefixo de nome longo demais cega a cascata inteira (adicionada 11/08/2026 — achado de CAPTURA).** `fn_norm_nome('DHL EXPRESS (BRASIL) LTDA')` = `DHLEXPRESSBRASIL`; a mesma empresa no espelho é `DHL EXPRESS (BRAZIL) LTDA` = `DHLEXPRESSBRAZIL`. Divergem no **14º caractere**. Qualquer comparação por `left(nome_k, N)` com **N ≥ 14 devolve false** — foi o que zerou o nível N4 do levantamento (0 linhas onde havia 7) e o que impediu **toda a DHL de casar por N1**: as 177 que casaram foram via N2 (número + janela de data), nunca por nome. O sintoma é traiçoeiro porque **nada falha** — o nível simplesmente devolve vazio e parece que não há o que casar. **Calibração medida (espelho jan–abr, 473 fornecedores distintos):** a colisão de prefixo **nunca zera** — em N=20 ainda restam 5 prefixos colidentes, porque há fornecedores cujo nome normalizado é prefixo de outro (`SCITECH PRODUTOS MEDICOS SA` × `... SA (SÃO PAULO)`; `LARA CRISTINA TARELHO DE OLIVEIRA` × `... CURI`). Curva: **N=12** → 10 prefixos / 23 fornecedores · **N=13** → 8 / 18 · **N=14** → 6 / 14 · **N=16–20** → platô em 5 / 10. **N=13 é o maior valor que ainda enxerga BRASIL/BRAZIL**, e o custo medido foi **zero**: das 8 colisões em 13, só 4 tocam o razão (13 linhas / R$ 144.817,97), **todas casaram por N1** — onde o número já garante a identidade e o nome apenas confirma — e 2 delas são a mesma entidade com grafia diferente, que é exatamente o que se quer colapsar. **Regra: prefixo de nome é parâmetro a CALIBRAR contra a base, nunca a escolher por intuição; e quando o número existe, é ele quem desambigua — o nome é confirmação, não chave.** Irmã da armadilha 15 (caixa inconsistente nas espécies): as duas dizem que **o cadastro do Alvo e o da contabilidade divergem em grafia, e todo casamento textual entre os dois precisa ser medido antes de ser confiado.**
...
|Data|Alteração|Por|
|-|-|-|
|11/08/2026|**Levantamento CC+Classe do razão contábil jan–abr/2026 (leitura pura, nenhuma escrita pelo agente).** Stage `desp_razao_conciliacao` criada e carregada pelo Pedro (1.443 linhas / R$ 6.762.644,41, conferido ao centavo). Cascata recalibrada para `left(nome_k,13)` + novo nível N4 (valor+nome+data ±5d): cobertura **86,0%** (1.014/1.179), contra 85,4% antes; o ganho maior não foi de cobertura e sim de **qualidade** — 155 linhas migraram de N2 para N1. Resíduo real isolado: **132 linhas / R$ 343.546,65** de despesa sem par no espelho (os −R$ 738k do SEM_MATCH bruto eram a soma líquida de 33 créditos de reembolso intercompany, que não têm par por natureza, com 139 débitos). Três achados de **CAPTURA** registrados como armadilhas **39** (dois padrões de numeração DHL no espelho), **40** (`chave_movestq` 100% nulo no DocFin) e **41** (BRASIL/BRAZIL + calibração do prefixo de nome). **Parte D (gravação de classes/centros) e marcação de natureza NÃO executadas** — seguem pendentes de aprovação|Pedro + Claude|
|22/07/2026|**Divergência de junho conferida e RESOLVIDA (83 admitidos vs 72 da §6):** o Alvo ganhou **9 docs** entre o censo de 13/07 (619) e hoje (628) — lançamento retroativo normal; reconciliação **360 anti-join + 47 legado + 221 FAT/sem chave = 628**. **Truncamento de índice descartado por evidência de direção:** o índice cresceu (5.122 → 5.178) e as admissões subiram junto — origem é o Alvo, não o índice. **Consequência:** a §6/censo é **retrato**, não previsão → **armadilha 38** + **critério de aceitação do backfill ajustado** (re-rodar o censo de cada competência imediatamente antes de reabri-la; número congelado deixa de ser critério duro). Jan–abr variam menos que junho (mais antigos), mas o princípio vale|Pedro + Claude|
...
|20/07/2026|**D-018 AUDITADO pelo Pedro — 2 pontos confirmados no `erp-proxy`:** `carregarIndiceMovEstq` (linhas 236–237) e limpeza de órfãos (949–953), ambos `.range()` **sem `.order()`**; `despesas.ts` não pagina. Correções definidas: `.order()` **pela PK** (`chave_movestq` / `chave_docfin`) antes do `.range()` — ordenar por coluna do `select` empataria e reintroduziria a instabilidade. **Baseline medido: índice = 5.139 linhas / 5.139 chaves distintas / 6 páginas de 1.000 — sinal de saúde permanente: se CAIR, quebrou** (e a consequência é dupla contagem silenciosa). **Teste de estabilidade** acrescentado ao passo 2: rodar o sync 2× e comparar `movestq_index_linhas`. **Armadilha 37** — *anotar o risco não é fechá-lo*: o comentário do código dizia que aquela paginação era "o ponto mais perigoso do arquivo" **e ela foi escrita sem `ORDER BY`**; diagnóstico certo, implementação incompleta (resolveu o truncamento, deixou a instabilidade), e o comentário de alerta **criou a sensação de que estava tratado**, impedindo a segunda pergunta. **Lista única de aplicação** consolidada em `SPEC-D016-passo2.md` (4 itens, um deploy só)|Pedro + Claude|
