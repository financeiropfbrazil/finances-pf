...
**Status:** ⬜ não iniciado · **Origem:** achado ao verificar a idempotência do backfill do D-001 (14/07/2026). · **Não bloqueia o D-001** (espécies-alvo não afetadas), mas corrompe o realizado.

Ao verificar se o re-sync do D-001 é seguro, descobri que **`desp_docfin_rateio` acumula grãos duplicados** para as espécies **RDESP e PC** — o motor grava o MESMO rateio (mesma `ordem_classe`/`ordem_rateio`/`codigo_classe`/valor) 2-3× **dentro de uma única rodada** (created_at com ~50ms de diferença, ex.: `chave_docfin=27324`, 3× R$ 19,76; `chave_docfin=26217` PC, 2× R$ 22.000). **Não** é falha de delete entre rodadas (o motor faz delete→insert corretamente — provado: mai/jun reprocessadas 33×/30× com razão 1.0). É gravação duplicada intra-rodada, específica de RDESP/PC.

Diagnóstico atual (jan–jun): **8 docs afetados, todos em abril** (competência que saiu da janela rolante em 16/06 antes de novo espelho): 7 RDESP (ent 0000934 FORNECEDORES DIVERSOS, valores micro) + 1 PC (ent 0001669 S A PERFORMANCE, +R$ 22.000). Inflação total ~R$ 22,3k, dominada pelo PC. As demais 29 espécies em `desp_docfin_doc` têm 0 duplicados.

Sequência:
1. 🔧 **Spec funcional para o Pedro** investigar no motor do erp-proxy por que RDESP/PC duplicam o grão intra-rodada (provável: o `DocFin/Load` dessas espécies retorna o rateio repetido, ou o loop de explosão itera 2× a mesma parcela). Corrigir na origem.
2. 🔧 **Limpeza dos 8 docs de abril** (escrita, com rito e WHERE revisado): deduplicar `desp_docfin_rateio` mantendo 1 grão por `(chave_docfin, ordem_classe, ordem_rateio)`. Alternativa: reprocessar abril **após** a correção do motor (o delete→insert limpa). Abril precisa estar ABERTA (checar trava de mês fechado).
3. 🔧 Considerar um `UNIQUE (codigo_empresa_filial, chave_docfin, ordem_classe, ordem_rateio)` em `desp_docfin_rateio` (espelho do `uq_desp_rateio_grao` do MovEstq) como blindagem estrutural — DDL, com aprovação.

*(novos cards entram aqui: D-007, D-008, ...)*
