 ...
 * Fallback: soma dos itens não cancelados — quando o Alvo NÃO fornecer total
 * (null/undefined). Um 0 explícito do Alvo é respeitado, EXCETO quando há itens
 * com valor (cabeçalho 0 + itens > 0 → usa a soma dos itens). Cobre o caso de
 * pedido "Em Andamento" cujo cabeçalho da listagem leve ainda não consolidou.
 */
...
export async function syncPedidosCompra(
  ...
  const { data: existing } = await supabase.from("compras_pedidos").select("numero, codigo_empresa_filial");
      ...
      await supabase.from("compras_pedidos").upsert(
        {
          ...
          // A listagem leve (GetListForComponents / mapPedido) grava ValorTotal=0
          // para pedidos em andamento. O Load completo traz o valor consolidado;
          // gravamos aqui para que a LISTAGEM fique consistente com o DETALHE.
          valor_total: resolverValorTotal(loadData, itens),
          valor_mercadoria: loadData?.ValorMercadoria ?? null,
