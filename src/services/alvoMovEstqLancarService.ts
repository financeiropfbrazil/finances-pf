...
function buildPayload(input: LancarNfseInput, anexos: { uuid: string; tipo: 'pdf' | 'xml' }[]): any {
  ...
  const dataMovimentoISO = hoje.toISOString();
  const dtEmissao = new Date(input.dataEmissao);
  dtEmissao.setHours(3, 0, 0, 0);
  const dataEmissaoISO = dtEmissao.toISOString();

  const valorServico = input.valorServico;
  const cnpjEntidade = input.prestadorCnpj.replace(/\D/g, "");
  const nomeEntidade = input.prestadorNome;
  const numeroNfse = input.numero;
  const codigoEntidade = input.codigoEntidade;
  const codigoCondPag = input.codigoCondPag;
  const codigoTipoPagRec = "0000016";
  const codigoProduto = input.codigoProduto;
  const nomeProduto = input.nomeProduto;
  const numeroPedComp = input.pedidoNumero;

  // Parcelas — do input ou fallback 1 parcela (emissão + 30 dias)
  let dataVencimentoISO: string;
  ...
  if (input.parcelas && input.parcelas.length > 0) {
    ...
    dataVencimentoISO = new Date(input.parcelas[0].dataVencimento + "T03:00:00.000Z").toISOString();
  } else {
    const dtVenc = new Date(dtEmissao);
    dtVenc.setDate(dtVenc.getDate() + 30);
    dtVenc.setHours(3, 0, 0, 0);
    dataVencimentoISO = dtVenc.toISOString();
    parcelasList = [{
      CodigoEmpresaFilial: "1.01",
      ChaveMovEstq: 1,
      Sequencia: 1,
      EspecieDocumento: "NFS-e",
      SerieDocumento: input.serie || "1",
      NumeroDuplicata: `${numeroNfse}/1-1`,
      DataEmissao: dataEmissaoISO,
      ValorParcela: valorServico,
      ValorPago: 0,
      DataPagamento: null,
      DataVencimento: dataVencimentoISO,
      DataProrrogacao: dataVencimentoISO,
      NumeroBanco: null,
      NumeroAgBancaria: null,
      CodigoTipoCobranca: "0000001",
      ParcPagMovEstqUserFieldsObject: {},
  ...
  // Classes + rateio centros de custo
  ...
  return {
    ...
    // === DATAS ===
    DataMovimento: dataMovimentoISO,
    DataEmissao: dataEmissaoISO,
    DataEntrada: dataMovimentoISO,
    ...
    ValorDocumento: valorServico,

    // === CONDIÇÃO DE PAGAMENTO ===
    CodigoCondPag: codigoCondPag,
    ...
    DataHoraDigitacao: new Date().toISOString(),
    ...
    CodigoTipoPagRec: codigoTipoPagRec,

    // === FRETE/SEGURO/IMPORTAÇÃO ===
    EntidadeFrete: null,
    ...
    BaseCSLLRFServico: valorServico,
    ValorCSLLRFServico: 0,
    PercentualCSLLRFServico: 1,
    DeduzCSLLValorTotal: "Não",
    PercentualCOFINSRFServico: 3,
    ValorCOFINSRFServico: 0,
    DeduzCOFINSValorTotal: "Não",
    PercentualPISRFServico: 0.65,
    ValorPISRFServico: 0,
    ...
    IntegraFiscal: "Não",
    ValorGlosa: 0,
    ValorLiquidoDocumento: valorServico,
    ValorPedagio: 0,
    CodigoEnderEnt: null,
    ...
    BasePISRFServico: valorServico,
    BaseCOFINSRFServico: valorServico,
    BasePISRFProduto: 0,
    ...
    Rascunho: "Não",
    NumeroDocumentoReferencia: numeroNfse,
    ...
    ItemMovEstqChildList: [{
      ...
      BaseCustoMedio: valorServico,
      CustoUnitario: valorServico,
      ...
      BaseCSLLRF: valorServico,
      PercentualCSLLRF: 1,
      ValorCSLLRF: 0,
      PercentualCOFINSRF: 3,
      ValorCOFINSRF: 0,
      PercentualPISRF: 0.65,
      ValorPISRF: 0,
      ...
      Patrimonio: "Não",
      CodigoProdutoPedComp: codigoProduto,
      SequenciaItemPedComp: 1,
      ValorDescontoGeral: 0,
      ...
      BasePISRF: valorServico,
      BaseCOFINSRF: valorServico,
      NaturezaFrete: null,
      ...
      ValorAnteriorExecucaoOrcamentaria: valorServico,
      AvisoRetorno: null,
      IDGeraNumSerie: 0,
    ...
    MovEstqPedCompChildList: [{
      ...
      CodigoEmpresaFilialPedComp: "1.01",
      NumeroPedComp: numeroPedComp,
      ValorItens: valorServico,
      ValorLiberado: 0,
      ValorOriginal: 0,
      CodigoEmpresaFilialDocFin: null,
      ChaveDocFin: null,
      MovEstqPedCompNecNovaProj: "Não",
      MovEstqPedCompValDocSaldo: valorServico,
