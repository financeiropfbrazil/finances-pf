...
export interface NfeItemInput {
  ...
  valorProduto: number;
  quantidade: number; // ⭐ NOVO: quantidade real do item da NF (era hardcoded 1)
  unidade?: string; // ⭐ NOVO: unidade comercial do XML (uCom)
  codigoNCM?: string;
...
function buildPayload(input: LancarNfeInput, anexos: { uuid: string; tipo: "pdf" | "xml" }[]): any {
  ...
  // Itens reais — agora com QUANTIDADE e UNIDADE reais (antes era hardcoded 1/UNID)
  const itemsList = input.itens.map((item, idx) => {
    const qtd = item.quantidade && item.quantidade > 0 ? item.quantidade : 1; // fallback defensivo
    const unid = item.unidade && item.unidade.trim() ? item.unidade.trim().toUpperCase() : "UNID";
    return {
      CodigoEmpresaFilial: "1.01",
      ...
      QuantidadeProdUnidMedPrincipal: qtd, // ⭐ quantidade real
      ValorProduto: item.valorProduto,
      CodigoProdUnidMed: unid, // ⭐ unidade real
      PosicaoProdUnidMed: 1,
      Peso: 1,
      ItemServico: "Não",
      Quantidade2: qtd, // ⭐ quantidade real
      ControlaEstoque: "Sim",
      CodigoTributA: "0",
      CodigoTributB: "90",
      CodigoClasFiscal: item.codigoClasFiscal || "0000002",
      CodigoProdUnidMedValor: unid, // ⭐ unidade real
      CodigoEntidade: input.codigoEntidade,
      NomeProduto: item.codigoProduto,
      QuantidadePatrimonio: qtd, // ⭐ quantidade real
      ...
      ValorUnitario: qtd > 0 ? Number((item.valorProduto / qtd).toFixed(5)) : item.valorProduto, // ⭐ unitário coerente
...
export async function lancarNfeNoAlvo(input: LancarNfeInput): Promise<LancarNfeResult> {
  ...
  const payload = buildPayload(
    input,
    anexos.map((a) => ({ uuid: a.uuid, tipo: a.tipo })),
  );

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    if (attempt === 1) clearAlvoToken();
    const auth = await authenticateAlvo();
    if (!auth.success || !auth.token) return { success: false, error: "Falha na autenticação ERP" };
    ...
    const resp = await fetch(`${ERP_BASE_URL}/MovEstq/SaveMovEstqMultPart?action=Insert`, {
      method: "POST",
    ...
    if (!data?.Chave || data.Chave === 0) return { success: false, error: "Resposta sem Chave" };
