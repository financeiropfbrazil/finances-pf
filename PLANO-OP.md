...
| AT-1 | Whitelist no erp-proxy: `CtrlLoteLocArmaz/ListaCtrlLoteLocArmaz` + `ReqMat/ValidarAtendimento` + `ReqMat/FinalizarAtendimento` | PENDENTE | — | Fase 4. Detalhamento em `PLANO-RM-ATENDIMENTO.md`. ⚠ Gargalo compartilhado (Suprimentos 100+ usuários); commit mínimo e canário depois do deploy. |
| AT-2 | Capturar `RelacionarCtrlLoteLocArmaz` | PENDENTE | — | Fase 4. Única incógnita restante: o atendimento por API da `2273` funcionou **sem** ele. |
...
### 2. ~~BL-21 — endpoint de lotes disponíveis~~ ✅ **FEITO em 08/08/2026**

~~Pré-requisito da tela de atendimento.~~ **Capturado em 08/08/2026** — endpoint, payload e resposta em §10.30 e no BL-21. ⇒ **A Fase 4 (atendimento pelo almoxarifado) deixa de ter incógnita e passa a ter plano próprio: `PLANO-RM-ATENDIMENTO.md`**, com as tarefas AT-1 a AT-6. O que resta dela é execução, não descoberta. ⚠ Uma incógnita menor ficou registrada lá como **AT-2**: o `RelacionarCtrlLoteLocArmaz` não foi capturado — o atendimento por API da `2273` funcionou sem ele, então ou é conveniência da tela, ou faz algo a mais que não notamos.
