...
| OP-2.5 | Estado da RM: exibir exclusão + agendar o cron + conferência sob demanda | **CONCLUÍDA** | 06/08/2026 | **✅ FECHADA em 07/08/2026 — as três pendências caíram:** redeploy feito (`detalhados=150` nas execuções automáticas, prova de que o `LOAD_BATCH` novo saiu no bundle), `sql/OP-2.5.sql` aplicado com o horário ajustado de `:15` para **`:25`** (as duas pontas em sincronia — `cron.job` jobid 25 e `sync_settings.schedule_cron`), e a inanição confirmada como resolvida: `fila_restante` **520 → 501 → 475 → 456 → 434 → 409 → 382**, caindo pela primeira vez. Detalhe em §10.27. ⚠ Dois defeitos no arquivo `sql/OP-2.5.sql` (bloco imutável) foram encontrados depois e estão registrados no cabeçalho do `sql/OP-2.6.sql`. **Texto original da entrega abaixo.** **Três entregas.** (1) 🔴 **O bug:** a RM `0000002271`, excluída no Alvo, aparecia na fila como viva — o espelho **sabia** (`ausente_desde` carimbado em 05/08 19:16), a **tela** é que não mostrava. `statusRM.ts` virou status EFETIVO no molde do `statusPedido.ts`: `getStatusRM` passou a receber a **linha**, nova chave `ausente_alvo` **primeira na precedência** (label "Excluída no ERP", `line-through`), filtro server-side `aplicarFiltroStatusRM` (os status do Alvo passam a exigir `.is("ausente_desde", null)`, senão filtro e badge divergem) e chip próprio que **só aparece com ≥1**. Badge do detalhe e do consolidado da OP acompanham. (2) **`LOAD_BATCH` 60 → 150** por **inanição por prioridade**: a fila ordena por tipo `asc` e '0000002' é o menor; com 120 RMs de produção permanentes na fila e teto 60, `120 >= 60` **sempre** e as ~400 de outros tipos nunca chegavam ao topo — visível em `sync_runs` (`fila_restante` 646→621→589→549→520→**520**, estacionou). `sql/OP-2.5.sql` descomenta o `cron.schedule` (`15 12,15,18,21 * * 1-5`). ⏸ **NÃO aplicado; exige REDEPLOY da função antes.** (3) **"Conferir no ERP"** no detalhe — `POST /alvo/passthrough` com o JWT do usuário, **só leitura, não persiste nada**, guarda anti-wipe de exibição, 412 vira aviso e **não marca ausência**. Validação: `tsc --noEmit` 0 erros, build limpo, `vitest run` 8/8. |
| OP-2.4 | Tela `Produção > RM` — LEITURA (fila + detalhe + consolidado na OP) | **CONCLUÍDA** | 06/08/2026 | **✅ FECHADA em 07/08/2026:** `sql/OP-2.4.sql` **aplicado** (verificado — a policy `op_requisicoes_select` está com `producao.access OR producao.rm.access`), tela validada e publicada. Detalhe das três medições que mudaram a implementação em §10.26. **Texto original da entrega abaixo.** **Código no preview; nada publicado, nada aplicado no banco, nada escrito no Alvo.** Novos: `src/lib/statusRM.ts` (3 literais do Alvo, fonte única), `src/services/reqMatService.ts`, `src/pages/ProducaoRM.tsx` (fila), `src/pages/ProducaoRMDetalhe.tsx` (detalhe por **número**, não uuid), `sql/OP-2.4.sql`. Editados: `App.tsx` (2 rotas, gate `producao.rm.access`), `AppSidebar.tsx` (sub-item "RM" com `perm`, `renderProducaoGroup` passou a receber `hasAccess` e a **não renderizar grupo vazio**), `constants/permissions.ts` (3 códigos espelhados), `ProducaoOrdemDetalhe.tsx` (seção de consolidado). Filtro-base **fixo e invisível** `codigo_tipo_req_mat='0000002'` em toda consulta. Validação: `tsc --noEmit -p tsconfig.app.json` **0 erros**, `bun run build` limpo, `vitest run` **8/8** (inclui os 7 do `sidebar-ordem`, que **não** precisaram de alteração — selam o topo, e o item novo é sub-item). ⏸ **`sql/OP-2.4.sql` NÃO aplicado** (RLS ampliada para `producao.access OR producao.rm.access`) — enquanto não for, nada muda: as duas permissões estão nos mesmos papéis. Criação de RM, botão "Nova RM" e RPC de escrita **fora do escopo** (Parte 3). |
| OP-2.6 | Cron de RM em Ferramentas — tela + registro do job no roteador | **CONCLUÍDA** | 07/08/2026 | **✅ FECHADA em 07/08/2026:** `sql/OP-2.6.sql` aplicado, tela no ar, as cinco telas antigas de cron conferidas. O achado central — **não foi preciso RPC intermediária** — está em §10.28. **Texto original da entrega abaixo.** **Código no preview; nada aplicado no banco, nada publicado.** (1) **`sql/OP-2.6.sql` — NÃO APLICADO.** Duas funções reescritas INTEIRAS (o `CREATE OR REPLACE` não preserva `SECURITY DEFINER` nem `SET search_path` — armadilha registrada): `_sync_cron_resolve` ganha `when 'reqmat'` mantendo `IMMUTABLE` e **sem** virar definer; `sync_cron_trigger_now` ganha `when 'reqmat' then call_sync_reqmat_cron('manual_admin')` com `SECURITY DEFINER` + `SET search_path = public, vault, extensions` reescritos por obrigação; `notify pgrst`. **Dependências conferidas (fingerprint 1819):** `sync_settings` com `job_name='sync-reqmat'` / `schedule_cron='25 12,15,18,21 * * 1-5'` / `enabled=true` / `paused_at=null`, e `sync_runs_job_type_check` já com `'reqmat'`. ⚠ **Aplicar FORA das janelas de cron** — :00, :10, :15, :25, :30 e :45 estão ocupados; janela segura em :05, :20, :35, :40, :50 ou :55. Rollback = as duas definições anteriores, comentadas no fim do arquivo. (2) **Tela** `src/pages/ferramentas/CronReqMatDashboard.tsx`, cópia do `CronIntercompanyDashboard` com `const JOB = "reqmat"`; rota `/ferramentas/cron-reqmat` (`permKey="ferramentas_cron_req"` ⇒ `ferramentas.cron.view`, gate único aprovado) e sub-item "Cron RM (Produção)" no grupo Ferramentas. 🔴 **O modal NÃO foi copiado** — `EtapaRow` próprio dirigido pela chave `etapa`, com `ausencia` em **âmbar** ("Requisições que sumiram do Alvo" / "nada foi apagado") e não no bloco vermelho de erro; rótulos do quadro de topo na língua do job ("Listadas no Alvo" / "Gravadas no espelho" / "RMs relidas" / "Erros"); duração em âmbar acima de **80 s** (as 3 execuções automáticas ficaram em 53–56 s contra watchdog de 110 s — folga de ~2×). `describeSchedule` hardcodado como as cinco ("09h25, 12h25, 15h25 e 18h25 (seg-sex)"), com a expressão crua exibida ao lado para a divergência não passar em silêncio. (3) **BL-27 e BL-28** no §8; adendo ao **BL-16**. Validação: `tsc --noEmit -p tsconfig.app.json` **0 erros**, `bun run build` limpo, `vitest run` **8/8** (o `sidebar-ordem` selado passa sem alteração — sub-item não muda a ordem de topo), eslint sem erro novo. **Contrapartida aceita:** com gate `ferramentas.cron.view` sozinho, Maria Eduarda não dispara da própria tela — com o cron 4×/dia e o botão "Conferir no ERP" no detalhe, a pergunta dela já tem resposta. |
| OP-2.7 | `Produção > RM` — CRIAÇÃO (modal + ciclo de 3 passos + livro) | **CONCLUÍDA** | 07/08/2026 | **✅ O PRIMEIRO TESTE REAL ACONTECEU em 08/08/2026 e o ciclo passou:** RM `0000002286`, `status_envio = 'confirmado'`, três passos em **10 s** (Insert 00:08:45 → Update 00:08:53 → Load 00:08:55), e no Alvo `Origem: "Importação"`, `TipoAtendimento: "Manual"`, `CodigoFuncionario: "0000149"`. 🔴 **E ela não apareceu em lugar nenhum da fila** — o que abriu a OP-2.8 e revelou um defeito de EIXO no bloco "Aguardando sincronização" (§10.29). O ciclo de criação em si está fechado. **Texto original da entrega abaixo.** **⚠ (Nota histórica: enquanto o teste não ocorreu, esta linha esteve como ENTREGUE.)** O que falta não é código: é a RM de verdade, na OP `2026-0503`, com usuário sem `is_admin` — é o passo 1 do §11. Enquanto não acontecer, o ciclo de três passos está provado **por fora** (RM `0000002285`, no Lab) mas nunca rodou **pela tela**. Receita medida em §10.24. **Texto original da entrega abaixo.** **Código no preview; `sql/OP-2.7.sql` NÃO aplicado; nada publicado; nenhuma RM criada no Alvo por esta sessão.** Fecha a Fase 2 do lado do Hub. **(1) SQL:** 3 RPCs `SECURITY DEFINER` + `SET search_path=public`, gate `producao.rm.create`, envelope `{success, mensagem, erro_codigo}` — `op_rm_criar` (valida OP ABERTA/EM_ANDAMENTO, grava `pendente` + `payload_enviado` + `criado_por`), `op_rm_marcar_enviado` (status decidido pelo NÚMERO, não pelo erro; recusa número divergente), `op_rm_marcar_confirmado` (só de `enviado`, só com número). Mais `ux_op_requisicoes_reqmat` — único parcial, a trava anti-duplicata que faltava. ⚠ **Nenhuma policy de INSERT/UPDATE para `authenticated`** — abrir a tabela seria a classe do BL-5, e é o padrão que Projetos abandonou no A-2. **(2) Modal** `NovaRMModal.tsx`: OP (só abertas — hoje 2026-0501 e 2026-0503), Descrição com prefixo `"OP 2026-0503 - "` **dentro do `maxLength`** (o limite de 40 é do Alvo, §6.2), Centro de custo de **`cost_centers`** (81 ativos, default `00009.00001.00001`), Texto (carregador humano da OP na tela de atendimento do Alvo) e Funcionário **fixo, do perfil** — sem de-para não se cria, no molde do gate do L7-B. Picker de SKU **importado** do `NovaOPModal` (que ganhou só um `export`), não duplicado. **10 validações antes do POST**; a #8 bloqueia no picker o produto sem `unidade_medida` (**155 de 2.806 ativos**, 5,5%) — é o candidato nº 1 ao `NullReferenceException` mudo do §6.3-N. **(3) Ciclo** em `alvoReqMatSaveService.ts`: livro → Insert (`Origem: "Importação"`) → Update (`TipoAtendimento: "Manual"`) → Load de confirmação, tudo por `POST /alvo/passthrough` com `{endpoint, method, payload}` — **nada no `erp-proxy`**. 🔴 **Estado perigoso tratado:** Insert OK + Update falho ⇒ `enviado` + número + `erro_mensagem`; a tela oferece **"Concluir envio (passo 2)"** (reexecuta só 2 e 3) e **bloqueia "Nova RM" naquela OP** — sem isso o operador tenta de novo e duplica a RM no ERP. **(4) Fila:** botão "Nova RM" (`producao.rm.create` — 3 pessoas hoje) e bloco **"Aguardando sincronização"** lendo o livro, porque o espelho só anda 4×/dia e a RM criada às 10h não apareceria até 12h25. **Receita medida em §10.24.** Validação: `tsc --noEmit` **0 erros**, build limpo, `vitest run` **8/8** (`sidebar-ordem` selado passa sem alteração), eslint sem erro novo nos arquivos preexistentes. |
| OP-2.8 | A RM criada no Hub aparece na hora — semeadura do espelho + guarda de ausência | **CONCLUÍDA** | 08/08/2026 | **A RM criada no Hub é semeada no espelho na hora, a partir do corpo do `ReqMat/Load`. Validada com usuário NÃO-admin.** ✅ **Aplicada, redeployada, publicada e PROVADA EM CAMPO em 08/08/2026:** a RM `0000002287` foi criada por `pscrignoli@gmail.com` (**`is_admin = false`**, papel `operador_producao`, funcionário `0000001`) e apareceu na fila **no mesmo instante do `confirmado_em` (01:16:07)**, com `origem = Importação`, `tipo_atendimento = Manual` e `ausente_desde` NULL — a guarda de recência segurou. Fecha a única peça que faltava desde 06/08, quando se descobriu que o Pedro é o único `is_admin` de 52 e por isso não conseguiria testar permissão. Detalhe em **§10.29**. **Texto original da entrega abaixo.** Nasceu do primeiro teste real da OP-2.7: o ciclo passou em 10 s e a RM **não apareceu** — nem na tabela (o espelho só anda 4×/dia; era sábado e o cron roda `* * 1-5`) nem no bloco "Aguardando sincronização", que filtrava `status_envio IN ('pendente','enviado')` e não alcançava `confirmado`. **(1) Mapper compartilhado** `supabase/functions/_shared/reqmatMapper.ts` — o mapper SAIU do `sync-reqmat` e passou a ser importado pelos dois mundos (Deno e Vite). É o que garante "gravar exatamente como o sync gravaria" **por construção**, não por disciplina. Medido antes de decidir: `tsc --noEmit -p tsconfig.app.json` e `vite build` aceitam o import de fora de `src/` sem configuração nova. **(2) `sql/OP-2.9.sql`** — `op_reqmat_semear_criacao`, `SECURITY DEFINER`, gate `producao.rm.create` **e** exigência de linha em `op_requisicoes` com `status_envio='enviado'`; insere só a chave do cabeçalho (com `sincronizado_em = now()`) e **delega itens, lotes e carimbos à `op_reqmat_aplicar_load`**, preservando a transação única daquela função — o REVOKE dela não impede a chamada porque a chamadora é DEFINER com owner `postgres` (mecanismo do §10.28, conferido no banco). **Nenhuma policy de INSERT em `op_reqmat`** — seria o BL-5 literal. **(3) Semeadura no passo 4**, dentro do `concluirEnvioRM`, **entre o Load e o `marcarRMConfirmada`** (a ordem é exigência: a RPC só aceita o livro em `enviado`). Semeia do **corpo do Load**, nunca do payload — 🔴 medido: o Hub enviou `Sequencia: 1` e o Alvo devolveu **`2`**; semear do payload gravaria PK errada e produziria lotes órfãos da FK no atendimento. Falha aqui **não derruba o fluxo**: avisa e o sync traz em ≤3 h. **(4) Guarda de recência no `sync-reqmat`** — linha do espelho criada DEPOIS do retrato da listagem não é avaliada para ausência. Sem ela a RM recém-criada seria marcada "Excluída no ERP" e apareceria riscada por até 3 h; o teto de 5% não protege de um caso isolado (limite ~34 num espelho de 688). ⚠ **Exige REDEPLOY da Edge Function.** **(5) Tela:** o bloco mudou de "Aguardando sincronização" (azul, informativo) para **"Envios incompletos"** (âmbar, acionável), perguntando o eixo certo — *tem linha no livro sem linha no espelho?* —, e em regime normal fica vazio e invisível; mais o sinal discreto "criada agora" na linha, derivado de `confirmado_em > sincronizado_em`, **sem coluna nova e sem DDL**, que se apaga sozinho no primeiro sync. `rmPendenteDeConclusao` preservada (é a trava anti-duplicata). Detalhe e a lição de eixo em **§10.29**. Validação: `tsc --noEmit -p tsconfig.app.json` **0 erros**, `bun run build` limpo, `vitest run` **8/8**, eslint **sem erro novo** nos arquivos de `src` (73/8/1/0, idênticos ao HEAD), type-check isolado do `sync-reqmat` com o **mesmo perfil do HEAD** (só módulo remoto + `Deno`). |
| AT-1 | Whitelist no erp-proxy: `CtrlLoteLocArmaz/ListaCtrlLoteLocArmaz` + `ReqMat/ValidarAtendimento` + `ReqMat/FinalizarAtendimento` | **CONCLUÍDA** | 09/08/2026 | **Deploy verde no Render e provado no console: 403 → 417/200/200** (§10.31). ⚠ Os três aceitam chamada de qualquer autenticado — o passthrough valida só JWT (BL-5). Remover se a Fase 4 parar. |
| AT-2 | Capturar `RelacionarCtrlLoteLocArmaz` | **CONCLUÍDA** | 09/08/2026 | 🟢 **É o alocador FEFO do SERVIDOR** — devolve a `CtrlLoteItemReqMatChildList` montada. O Hub não precisa implementar FEFO. Payload/response em §10.31. |
| AT-3 | RPC de escrita do atendimento + livro — `sql/AT-1.sql` | PENDENTE | — | Fase 4. Gate `producao.rm.atender`; ⚠ `revoke execute … from anon` com assinatura completa (§6.3-N). |
| AT-4 | Serviço do ciclo: Load → lotes → **releitura** → Validar → Finalizar → semear | PENDENTE | — | Fase 4. 🔴 A releitura antes de finalizar é requisito (§10.30). |
| AT-5 | Tela/modal de atendimento, gate `producao.rm.atender` | PENDENTE | — | Fase 4. FEFO pré-preenchido, contador Alocado/A atender/Diferença, campos de Entrega. |
| AT-6 | Registro da Fase 4 no `PLANO-OP.md` | PENDENTE | — | Fase 4. |
...
**Achados de 08/08/2026 (OP-2.8)**

🔴 **REGRA DO PROJETO, NÃO EXCEÇÃO: `revoke ... from public` NÃO tranca RPC nova.**

O Supabase mantém, para funções em `public`, um
`ALTER DEFAULT PRIVILEGES ... GRANT EXECUTE ON FUNCTIONS TO anon, authenticated, service_role`
— confirmado lendo `pg_default_acl`:

```
defaclrole | defaclnamespace | defaclobjtype | defaclacl
postgres   | public          | f (function)  | {postgres=X/postgres, anon=X/postgres,
                                                authenticated=X/postgres, service_role=X/postgres}
```

⇒ **Toda função nova criada em `public` nasce com EXECUTE concedido NOMINALMENTE a `anon`** —
não herdado de PUBLIC. E `revoke ... from public` **não alcança um grant nominal**: ele remove o
grant de PUBLIC, que aqui nem existe, e deixa o de `anon` intacto.

⇒ **Só `revoke execute on function ... from anon`, com a assinatura completa, funciona.**

Foi exatamente o que aconteceu na **OP-2.7**: o arquivo trazia `revoke ... from public` e as três
`op_rm_*` nasceram **abertas a `anon` mesmo assim** — foi preciso corrigir à mão, por função.
⚠ O `revoke` tem de vir **depois** do `create or replace`: um replace sobre função existente
...
|---|---|---|---|
| §6.3-N (05/08) | `item.BaixaEstoque` = **regra** ("este item baixa quando atendido") | `cabecalho.BaixouEstoque` = **fato** ("já baixou") | Concluir que o material saiu quando ele está no estoque |
| §10.27(a) (06/08) | `op_reqmat.ausente_desde` = o **espelho** sabia | a **tela** não perguntava | RM excluída no ERP exibida como viva |
| §10.29 (08/08) | `status_envio` = "o **ERP** aceitou?" | presença em `op_reqmat` = "o **Hub** já viu?" | RM `confirmado` invisível nas duas vistas |

⇒ **Onde há livro e espelho, todo filtro de UI declara qual dos dois está perguntando.** O
formato se repete: dois eixos, nomes parecidos, um lido pelo outro — e o erro é **silencioso e
plausível**, porque nada falha; o número continua parecendo certo.

⇒ E o motivo de o último só aparecer agora: **um caminho feliz que nunca rodou não é caminho
validado.** O defeito surgiu no primeiro `confirmado` da história do módulo — os testes
anteriores nunca chegaram ao estado terminal.
...
| 06/08/2026 | OP-2.5 · estado da RM | **O bug era de TELA, não de dado — e a investigação achou um problema maior no cron.** A RM `0000002271`, excluída no Alvo em 05/08, seguia na fila como viva; o espelho já tinha `ausente_desde` carimbado às **19:16 de 05/08** pelo próprio `sync-reqmat`. ⇒ **O dado estava certo o tempo todo.** Corrigido no `statusRM.ts`, que virou status EFETIVO no molde do `statusPedido.ts` (a ausência **precede** o literal do Alvo, como `excluido_alvo` precede tudo no pedido), com o filtro server-side espelhando a precedência — sem isso o usuário filtraria "Atendida Total" e receberia linha riscada. 🔴 **ACHADO PRINCIPAL — inanição por prioridade no passo B.** A fila ordena por `codigo_tipo_req_mat asc NULLS LAST` e `'0000002'` é o MENOR dos quatro códigos; como RM não-terminal nunca sai da fila (desenho), as **120** RMs de produção em aberto consumiam o lote inteiro a cada execução: com `LOAD_BATCH=60`, **`120 >= 60` sempre**, e as ~400 de outros tipos **nunca** chegavam ao topo. Não é atraso, é fome — e está visível em `sync_runs`: `fila_restante` desceu 646→621→589→549→520 e **estacionou em 520**. O que eu havia registrado como "platô previsto pelo desenho" em 05/08 era **metade platô, metade inanição**. Teto subido para **150** (as 120 cabem e sobram ~30/rodada), com a conta contra o watchdog: 150÷4 = 38 chunks × ~620 ms ≈ 24 s + passo A ≈ **28 s contra 110 s** ⇒ ~4× de folga; as 6 execuções reais de 05/08 levaram 20,6–25,5 s com teto 60 e `total_erros=0`. **Medição que reordenou as prioridades:** das 400 RMs nunca lidas, **ZERO são do tipo produção** — a tela de RM está com **280/280** detalhadas, e as 400 são consumo/ajuste/sem tipo, invisíveis nesta tela. Por isso o cron **não** era o conserto do bug (ele já tinha feito a parte dele), e o refresh sob demanda **não** era remendo de fila parada: são três problemas distintos. 🔴 **O cron nunca foi agendado** — as 6 execuções em `sync_runs` são todas `manual_admin`, e `cron.job` (11 jobs ativos) não tem nenhum de reqmat. `sql/OP-2.5.sql` descomenta o bloco da §6 da OP-2.3; ⚠ **o redeploy da função vem ANTES**, senão o cron roda com o teto antigo e o agendamento parece não resolver. **Conferência sob demanda entregue pela opção mais barata das três avaliadas:** o front chama `ReqMat/Load` pelo `/alvo/passthrough` com o **JWT do usuário** (funciona para não-admin — é a pendência do BL-5 a nosso favor) e **não persiste nada**. Isso descarta por construção o wipe silencioso (não existe caminho de escrita), dispensa `grant execute` da `op_reqmat_aplicar_load` a `authenticated` (que exporia DELETE+INSERT no espelho a 100+ usuários, pior que BL-5/BL-17) e dispensa Edge Function nova — que exigiria extrair o miolo do `sync-reqmat` para `_shared/`, porque **aquele módulo tem `Deno.serve` no topo** e importá-lo arrastaria o servidor junto. A guarda estrutural (`analisarRespostaReqMat`, 4 degraus) foi **replicada** com comentário apontando a origem, exatamente por isso. **412 confirmado puro:** o `/alvo/passthrough` **não mascara**, ao contrário da rota `/ped-comp`, que converte em 404 por regex — duas rotas do mesmo gateway com caras diferentes para o mesmo erro, agora registrado dos dois lados. 🔴 **O refresh NÃO marca ausência**, e a razão tem nome: no módulo de Suprimentos a primeira versão marcava exclusão em qualquer 404 e **marcou 7 pedidos vivos**. Quem marca é o `sync-reqmat`, que tem o cross-check e já provou na 2271. **Três dívidas de OUTRO módulo registradas de passagem: BL-24** (404 sem cross-check em requisições — o erro dos 7 pedidos, vivo por outra porta), **BL-25** (mesmo caminho sem guarda anti-wipe: 200 com corpo vazio grava `sincronizada`) e **BL-26** (`authenticateAlvo` lê credencial do localStorage ⇒ open-load de pedidos só funciona para quem a tem — um dos motivos de a RM usar o passthrough). |
...
| 08/08/2026 | OP-2.8 · a RM aparece na hora | **O primeiro teste real da OP-2.7 passou — e revelou um defeito de EIXO que vale mais que a correção.** A RM `0000002286` percorreu os três passos em **10 s**, ficou `confirmado`, e no Alvo veio `Origem: "Importação"` + `TipoAtendimento: "Manual"`. 🔴 **E não apareceu em lugar nenhum:** nem na tabela (o espelho anda 4×/dia; era sábado e o cron roda `* * 1-5`) nem no bloco "Aguardando sincronização", que filtrava `status_envio IN ('pendente','enviado')`. **`confirmado` quer dizer que o ALVO aceitou, não que o ESPELHO viu** — o filtro perguntava um eixo e precisava do outro. É a **terceira ocorrência da mesma família** (§10.27(a): o espelho sabia da exclusão, a tela não perguntava; §6.3-N: `item.BaixaEstoque` é regra e `cabecalho.BaixouEstoque` é fato). ⇒ **Onde há livro e espelho, todo filtro de UI declara qual dos dois está perguntando.** E o motivo de só aparecer agora: **um caminho feliz que nunca rodou não é caminho validado** — nenhum teste anterior tinha chegado ao estado terminal. **A correção:** a RM criada pelo Hub passa a ser gravada no espelho **no ato**, a partir do corpo do `ReqMat/Load` do passo 3. 🔴 **Do Load, nunca do payload — medido:** o Hub enviou `Sequencia: 1` e o Alvo devolveu **`2`**; como a sequência é PK do item e é o que os lotes referenciam, semear do payload gravaria PK errada e produziria lotes órfãos da FK no atendimento. **Um mapper, dois mundos:** o mapper saiu do `sync-reqmat` para `_shared/reqmatMapper.ts` e é importado por Deno **e** Vite — "gravar exatamente como o sync gravaria" vira propriedade de construção, não de disciplina; medido antes de decidir que `tsc` e `vite build` aceitam o import de fora de `src/`. 🟢 **E as duas rotas do Load têm o MESMO contrato** — 103 chaves (navegador, JWT do usuário) contra 97 (`raw` do sync, `X-System-Secret`), diferença = exatamente os 6 arrays que o sync remove; **zero chave escalar divergente**. A credencial não muda o corpo. 🔴 **Efeito colateral que quase passou:** com linha no espelho que a listagem ainda não conhece, `marcarAusencias` a marcaria "Excluída no ERP" — e o teto de 5% **não protege** um caso isolado (limite ~34 em 688). Autocorrigiria na passada seguinte, o que é pior: **erro que se conserta sozinho é erro que ninguém investiga.** Guarda de recência entrou no mesmo pacote (exige redeploy). **Achado de segurança que vale para todo o projeto:** o `revoke ... from public` da OP-2.7 não surtiu efeito porque `pg_default_acl` tem `GRANT EXECUTE ON FUNCTIONS TO anon, authenticated, service_role` — **toda função nova nasce com EXECUTE nominal para `anon`**, e só `revoke ... from anon` com assinatura completa o remove. Não é higiene das 8 do BL-27: **é o comportamento-padrão do projeto**, e vale para toda RPC futura (§6.3-N). ✅ **FECHADA NO MESMO DIA:** SQL aplicado, Edge redeployada, publicada — e **validada com usuário NÃO-admin**, que era a peça que faltava desde 06/08. A RM `0000002287`, criada por `pscrignoli@gmail.com` (`is_admin = false`, `operador_producao`, funcionário `0000001`), apareceu na fila **no mesmo instante do `confirmado_em` (01:16:07)**, com `ausente_desde` NULL — a guarda de recência segurou. Quatro gates em série exercitados sem bypass (`producao.rm.access` → `producao.rm.create` → `op_rm_criar` → `op_reqmat_semear_criacao`), e de quebra a prova do funcionário fixo: duas contas, dois funcionários (`0000149` e `0000001`), cada RM com o seu — sai do perfil, não do código. Detalhe em **§10.29**. |
| 09/08/2026 | AT-1 e AT-2 · a Fase 4 perde a última incógnita | **Os dois endpoints de lote estão mapeados e a whitelist liberada — o que resta da Fase 4 é construção, não descoberta.** **(a) AT-1:** três entradas novas no `erp-proxy` (`CtrlLoteLocArmaz/ListaCtrlLoteLocArmaz`, `ReqMat/ValidarAtendimento`, `ReqMat/FinalizarAtendimento`), deploy verde, provado no console: **403 → 417/200/200**. 🔴 **Armadilha nova:** `FinalizarAtendimento` com payload vazio responde **200 com objeto ReqMat em branco**, não erro ⇒ o Hub não pode tratar 200 como sucesso; exigir a âncora `ReqMat.Numero`. **(b) AT-2 — o achado do dia:** o `RelacionarCtrlLoteLocArmaz` **não é conveniência da tela, é o alocador FEFO do servidor**. Recebe o item + a lista de lotes e devolve o item com a `CtrlLoteItemReqMatChildList` **já montada** (espécime `2277`: 20 un no lote `0002467`, o de validade mais próxima). ⇒ **O Hub não precisa implementar FEFO** — e continua podendo montar à mão, como na `2273`. Explica o episódio de 05/08 em que a quantidade do modal vinha pré-preenchida. **(c) 🟢 `QuantidadeBruta` RESOLVIDO** (a §10.20 o registrava como "significado não fechado"): **é a quantidade ORIGINAL do lote**, não o total do atendimento — `2277`/`0002467` = 350 original, 154 de saldo. Fecha nos três espécimes. **(d) BL-30 aberto:** o item traz `ItemReqMatClasseRecdespChildList` (`11.03`, 100%) que nenhum `Load` mostrava — classificação contábil que a RM do Hub pode estar nascendo sem. **(e)** Captura sem efeito colateral: log confirma `Lista` → `Relacionar` → `ExpireSession`, **sem Validar nem Finalizar** — a tela foi fechada no X. É o protocolo que evita repetir o BL-29. Detalhe completo em **§10.31**. |
| 08/08/2026 | BL-21 · atendimento e decisão de processo | **A última incógnita técnica da Fase 4 caiu — e a decisão do Pedro sobre o processo mudou um requisito.** **(a) BL-21 FECHADO:** o endpoint de lotes é `POST /api/CtrlLoteLocArmaz/ListaCtrlLoteLocArmaz`, com o item da RM inteiro em `ClassInstance` + contexto no topo (`Origem: 7`, `OperacaoLote: "Saída"`, `OperacaoRM: "Retirada"`). Resposta traz nº do lote, validade, **`DataFabricacao`** (que a tela do Alvo não mostra) e saldo — **já ordenada por validade**, ou seja, o Alvo entrega em FEFO. ⚠ `QuantidadeReserva/EmpenhoLote` **nulos** ⇒ saldo **bruto**. Detalhe em **§10.30**. **(b) DECISÃO DE PROCESSO:** o almoxarifado **pode atender no Hub OU no Alvo** — revisão consciente da §6.1-1, possível porque a §10.19 provou o atendimento por API. 🔴 **Isso torna a RELEITURA antes de finalizar um requisito, não um refinamento:** se alguém atender no Alvo entre a abertura da tela e o envio, o Hub manda quantidade já consumida — **e o Alvo aceita acima do pedido**, então não haveria erro, haveria **baixa em dobro** (o BL-18 por outra porta). ⚠ **Risco de adoção registrado:** Hub opcional = ninguém migra; recomendação é convivência **com data para fechar a porta do Alvo**. **(c)** A inspeção de recebimento **continua no Alvo, pela Qualidade** — e o papel `almoxarifado` **não** dá acesso à Fila de Inspeção. **(d) BL-29 aberto:** a captura custou o atendimento real da `0000002283` — 30 un de `001.003.00059` baixaram (674 → 644). **(e)** A Fase 4 ganhou plano próprio: **`PLANO-RM-ATENDIMENTO.md`**, tarefas AT-1 a AT-6. |
| 07/08/2026 | Fechamento do dia | **Três entregas e um achado que muda como se lê o gateway.** **(1) OP-2.6** — cron da RM em Ferramentas, e o achado de que **não era preciso RPC intermediária**: `sync_cron_trigger_now` é SECURITY DEFINER com owner `postgres`, então a chamada interna a `call_sync_reqmat_cron` roda pelo owner e o REVOKE não a alcança; duas linhas de SQL, e status/histórico/pausar/despausar passaram a servir a RM de graça, com o disparador ainda trancado (§10.28). **(2) OP-2.7** — a criação, com a receita medida no passthrough pela RM `0000002285` (§10.24): o passthrough **aceita POST com body** ⇒ nada no `erp-proxy`; envelope **limpo** (`data.Numero`), sem o eco bugado do `PedComp`; e **`Origem: "Importação"` é gravável E sobrevive ao Update** ⇒ a métrica de vazamento da §10.7 nasce funcionando, com o espelho em `ManualAlvo` 688/688. **(3) O `pg_cron` da RM está PROVADO** — quatro execuções automáticas, 48,7 a 57,4 s, zero erros, `detalhados=150`, fila drenando 456 → 434 → 409 → 382. ⚠ **A folga contra o watchdog é ~2×, não 4×.** 🔴 **Duas perdas registradas para não virarem descoberta cara depois:** o `SaveReqMat` **não tem `CodigoUsuario`** (ao contrário do `PedComp`), então toda RM do Hub gravará a **conta de serviço do Render** na coluna Usuário do Alvo — a identidade real fica em `CodigoFuncionario` + `op_requisicoes.criado_por`; e **existem DUAS tabelas de centro de custo com dados contraditórios** (`cost_centers` × `rh_centros_custo`), onde o mesmo código tem nome e flag de ativo diferentes, e o default em uso corrente nem existe na segunda. **E o passo 2 deixou de ser hipótese:** o universo passou de 13/266 para **16 `Automático` todas Abertas contra 271 `Manual` nenhuma aberta**, sendo a mais nova criada pela MARIA.EDUARDA **na tela do Alvo** — três RMs nasceram mortas em dois dias, e o defeito é do campo, não da API. |
| 06/08/2026 | Fechamento do dia | **O dia em que o módulo saiu do invisível.** 🔴 **O achado que explicava tudo:** a Fase 1 estava entregue e publicada desde 23/07, e `op_ordens` tinha 2 registros — não por falta de adesão, mas porque **ninguém tinha papel de produção** (zero atribuições em 52 usuários) e **`producao.access` era órfã do papel `admin`**. O chão de fábrica simplesmente não conseguia abrir a tela. Corrigido: 13 permissões órfãs mapeadas (42→55), 3 permissões `producao.rm.*` criadas já ligadas ao `admin`, e **três papéis com segregação de funções provada** — quem requisita não atende, quem atende não requisita (§10.25). Papéis atribuídos a maria.santos@ (= MARIA.EDUARDA, `0000098`) e ryan.santos@. ⚠ Achado de tabela: o perfil do Pedro apontava para **Adna Ferreira** em vez de Pedro Scrignoli — falha silenciosa que só apareceria num documento com o nome errado. **Entregas:** **OP-2.4** (tela de RM — fila, detalhe e consolidado, com três medições feitas ANTES de codificar, §10.26) e **OP-2.5** (`ausente_desde` visível na tela, que já estava certo no dado; refresh só-leitura pelo passthrough; e o cron). 🔴 **A fila do sync não estava em platô — estava em inanição por prioridade:** ordenação por tipo `asc` + ~120 RMs de produção permanentemente na fila + teto 60 ⇒ `120 ≥ 60` sempre, e as ~400 de outros tipos nunca chegavam ao topo. Teto para 150 e a fila **caiu pela primeira vez em sete execuções** (§10.27). |
| 07/08/2026 | OP-2.7 · criação de RM | **A Fase 2 fecha do lado do Hub: a RM passa a nascer aqui.** Receita completa e medida em **§10.24** — envelope limpo (`data.Numero`), `Origem: "Importação"` sobrevivendo ao passo 2, campos de empenho e `CodigoLocArmaz` confirmados, e o passthrough aceitando POST com corpo (⇒ **nada no `erp-proxy`**). **(a) O passo 2 virou urgência empírica:** o universo de produção passou de 13 `Automático`/266 `Manual` (05/08) para **16/271** (07/08), com as 16 `Automático` **todas Abertas** e as 271 `Manual` **nenhuma** aberta — e a mais nova, a `0000002283`, foi criada pela **MARIA.EDUARDA na tela do Alvo**, não por API. Três RMs nasceram mortas em dois dias; o defeito é do campo (BL-19), e o Hub agora corrige o próprio caso com o `Update` obrigatório. **(b) O estado perigoso tem saída, e a saída é a parte que importa:** Insert OK + Update falho deixa a RM viva no ERP e morta na prática; a tela oferece "Concluir envio (passo 2)" e **bloqueia "Nova RM" naquela OP**, porque o reflexo natural do operador duplicaria a requisição. **(c) Escrita só por RPC:** `op_requisicoes` continua sem policy de INSERT/UPDATE — o `.upsert()` direto do frontend, que `requisicoesService.ts` ainda usa em Suprimentos, é o padrão que Projetos abandonou no A-2 (o pedido entrava no ERP e o Hub devolvia `success:true` sem gravar nada). **(d) Duas armadilhas de dados encontradas e fechadas em comentário no código:** existem DUAS tabelas de centro de custo e elas **divergem no mesmo código** — `00001.00005.00002` é "PRODUCAO VALVULAS"/inativo na `cost_centers` e "PRODUCAO PERICARDIO"/ativo na `rh_centros_custo`, e o default em uso corrente (`00009.00001.00001`, 105 RMs) **nem existe** na segunda; e **155 dos 2.806 produtos ativos (5,5%) estão sem `unidade_medida`**, o candidato nº 1 ao `NullReferenceException` mudo do §6.3-N — bloqueado no picker, com o nome do produto na mensagem. **(e) Perda registrada:** `CodigoUsuario` não existe no contrato do `SaveReqMat`, então toda RM do Hub gravará a **conta de serviço** do gateway na coluna "Usuário" do Alvo; a identidade real fica em `CodigoFuncionario` + `op_requisicoes.criado_por` (§10.24). ⏸ **`sql/OP-2.7.sql` NÃO aplicado.** Nenhuma RM foi criada no Alvo nesta sessão — o ciclo já estava provado. |
| 07/08/2026 | OP-2.6 · cron de RM em Ferramentas | **O `pg_cron` da RM está PROVADO, e o gate da tela nova custou duas linhas de SQL.** (a) **Três execuções automáticas, zero erros** — 06/08 21:25 (53,1 s), 07/08 12:25 (55,7 s), 07/08 15:25 (56,4 s), todas `triggered_by='pg_cron'` com `detalhados=150`; a fila drenou 475 → 456 → 434 (~21/execução). A inanição por prioridade acabou: o `LOAD_BATCH=150` saiu no redeploy. ⚠ **A folga contra o watchdog é ~2×, não os ~4× estimados** no comentário da Edge Function (53–56 s contra 110 s) — daí o destaque âmbar de duração acima de 80 s na tela nova. (b) **Não há RPC intermediária a criar.** `sync_cron_trigger_now` é `SECURITY DEFINER` com owner `postgres`, então a chamada interna a `call_sync_reqmat_cron` roda com os privilégios do owner e o `REVOKE` de anon/authenticated **não a alcança** — o disparador segue trancado para o usuário final e o botão "Rodar Agora" funciona. Medido: os três (`_sync_cron_resolve`, `sync_cron_trigger_now`, `call_sync_reqmat_cron`) têm `proowner='postgres'`. É o mecanismo que sustenta o **BL-27**. (c) **Gate: `ferramentas.cron.view` sozinho** — as seis rotas de cron usam `permKey="ferramentas_cron_req"`, que o `usePermissions` traduz para esse código; nenhuma consta do `routePermMap` (o menu é gateado no grupo Ferramentas), então a nova não entra lá. (d) 🔴 **O modal foi reescrito, não copiado.** As telas antigas leem `detalhes` como lista de mudanças (`{tipo:'req'|'ped', numero, status_novo}`) e pintam de vermelho pela presença de `erro`. O `sync-reqmat` grava `{etapa, info?, erro?}` — e **a entrada de `ausencia` usa `erro` para dar DESTAQUE, não para sinalizar falha** (confirmado no banco: `{"erro":"1 requisição(ões) sumiram da listagem do Alvo — marcadas com ausente_desde (nada foi apagado)","info":"números (até 20): 0000002266"}`). Copiado sem tocar, o modal não quebraria — **mentiria**: badge "Ped" com número vazio e o achado mais importante do lote no bloco de erro. `EtapaRow` próprio, dirigido pela chave `etapa`: `lista`/`gravacao`/`load` informativos, `ausencia` e `watchdog` **âmbar**, `erro` de verdade em vermelho. (e) **Rótulos do quadro de topo renomeados** — "candidatos/consultados" mente aqui (são as ~685 da listagem do ano, não candidatas a mudança) e `total_mudaram` é "RMs relidas", não "mudaram de status". (f) **Achado de passagem:** o bloco de rollback do `sql/OP-2.5.sql` sugere `sync_cron_pause(p_job => 'sync-reqmat')` — `p_job` é o **alias**, não o `job_name`, então aquela linha falha hoje com "Job desconhecido"; depois da OP-2.6 a forma correta (`'reqmat'`) passa a existir. Registrado no cabeçalho do `sql/OP-2.6.sql`; o arquivo da OP-2.5 não foi alterado (bloco registrado é imutável). ⏸ **`sql/OP-2.6.sql` NÃO aplicado** — Pedro aplica fora das janelas de cron (:00, :10, :15, :25, :30, :45 estão ocupados; janela segura em :05, :20, :35, :40, :50 ou :55). |
...
| ID | Origem | Descrição |
|---|---|---|
| BL-29 | 08/08/2026 | ⚠ **A captura do BL-21 custou um atendimento REAL.** A RM `0000002283` (MARIA.EDUARDA, 07/08, "Insumos Produção") foi atendida integralmente durante a captura: **30 unidades de `001.003.00059` (SORO FISIOLÓGICO 0,9% BOLSA 1000 ML) baixaram do estoque**, do lote `0002467`, saldo **674 → 644**. **Verificar com o almoxarifado se o material foi fisicamente entregue à produção.** Se não foi, o sistema afirma uma entrega que não ocorreu — e o estorno tem a ressalva do BL-22 (pode não devolver tudo: 1 galão de 9 não voltou). **Lição de método:** capturar payload em tela de escrita exige fechar o modal ANTES do OK — o log já tem o que se quer no instante em que a tela abre. |
| BL-30 | AT-2 · 09/08/2026 | ⚠ **Classificação contábil no item da RM — campo novo, não mapeado.** O item devolvido pelo `RelacionarCtrlLoteLocArmaz` traz `ItemReqMatClasseRecdespChildList` preenchida (`CodigoClasseRecDesp: "11.03"`, `Percentual: 100`) — **não aparecia em nenhum `ReqMat/Load` anterior** (na `2251` vinha vazia). É rateio por classe de receita/despesa. 🔴 **Se a RM criada pelo Hub nascer sem isso, pode ficar sem classificação contábil.** Verificar: (a) de onde vem a classe — produto, centro de custo ou tipo de RM; (b) se as RMs criadas pelo Hub em 08/08 (`2286`, `2287`) a têm. Assunto de Controladoria, e entra na AT-4 se o mapper precisar enviá-la. |
...
| ~~BL-21~~ | 05/08/2026 | ✅ **FECHADO em 08/08/2026 — endpoint capturado com payload e resposta.** ~~Capturar o endpoint que lista LOTES DISPONÍVEIS: a tela "Seleção Lote (Saída)" mostra nº, validade e saldo, e o `FiltrarSaldoProduto` NÃO devolve saldo por lote (§9.9). A chamada dispara quando o modal abre, ANTES do `ValidarAtendimento`, e escapou de três tentativas de captura.~~ **É `POST /api/CtrlLoteLocArmaz/ListaCtrlLoteLocArmaz`.** Payload: o item da RM inteiro em `ClassInstance` + contexto no topo (`Origem: 7` — constante mágica —, `Data`, `DataMovimentacao`, `EspecieDocumento`, `NumeroDocumento`, `SerieDocumento`, `SequenciaDocumento`, `OperacaoLote: "Saída"`, `OperacaoRM: "Retirada"`, `CodigoTipoLanc: ""`). Resposta: `{ListaCtrlLoteLocArmaz: [...]}` com `NumeroCtrlLote`, `DataValidadeCtrlLote`, **`DataFabricacao`** (que a tela do Alvo não mostra) e `QuantidadeSaldoProdUnidMedPrincipal`. 🟢 **Vem ordenada por validade** ⇒ o Alvo já entrega em FEFO, o que torna barata a sugestão automática. ⚠ `QuantidadeReservaLote`/`QuantidadeEmpenhoLote` **nulos** ⇒ o saldo é **bruto**, não descontado de reserva/empenho. ⚠ **Não capturado:** `RelacionarCtrlLoteLocArmaz` — vira **AT-2** no `PLANO-RM-ATENDIMENTO.md`. **Detalhe completo em §10.30.** |
...
### 10.24 — Criação de RM pelo Hub · receita medida e o que ela custa (07/08/2026)

**Todas as incógnitas do desenho foram fechadas em campo**, na RM `0000002285` (criada, atualizada, relida e depois excluída). Nada aqui é dedução.

**Caminho.** `POST {gateway}/alvo/passthrough` com o JWT do usuário e corpo `{ endpoint, method, payload }` — o `alvo.ts` faz `const { endpoint, method, payload } = req.body` e repassa a `callAlvo(endpoint, method, payload)`. ⇒ **O passthrough aceita POST com corpo, e `ReqMat/SaveReqMat` já está na whitelist. Nenhuma entrega no `erp-proxy`.**

**Envelope da resposta — LIMPO nesta rota.** `r.data` é o objeto ReqMat direto e o número está em **`data.Numero`** (`"0000002285"`). As 10 primeiras chaves: `CodigoEmpresaFilial · Numero · Data · Descricao · CodigoCentroCtrl · CodigoFuncionario · EspecieDocumento · SerieDocumento · NumeroDocumento · Status`. ⚠ **Não há o eco bugado do `PedComp`** (§6.2), onde o `Numero` é replicado em todos os campos string. Ainda assim lemos só `Numero` — a verdade final vem do `Load`.

**`Origem` SOBREVIVE ao passo 2.** Insert com `Origem: "Importação"` → Update com `TipoAtendimento: "Manual"` → `ReqMat/Load` devolveu **as duas coisas juntas**. ⇒ A métrica de vazamento da §10.7 está preservada: hoje o espelho tem `ManualAlvo` em **688/688**, então toda RM nascida no Hub fica distinguível para sempre.

**Campos que o teste fechou:** `CodigoAlternativoProduto` preenchido é aceito; repetir a quantidade pedida em `QuantidadeEmpenharProdUnidMedPrincipal`, `QuantidadeSaldo2` e `QuantidadeEmpenhar2` funciona; `CodigoLocArmaz: "001"` fixo no cabeçalho **e** no item é aceito no Insert.

> 🔴 **O passo 2 não é precaução teórica — piorou entre 05 e 07/08.** Universo de produção em 07/08 (n=287): **`Automático` = 16, TODAS `Aberta`; `Manual` = 271, ZERO aberta.** Em 05/08 era 13/266. **Três RMs nasceram mortas em dois dias**, e uma delas é a `0000002283`, criada pela **MARIA.EDUARDA na TELA do Alvo** em 07/08 17h46 — ou seja, **o defeito é do campo, não da API** (é o BL-19 acontecendo). O Hub não pode herdá-lo: sem o Update, a RM fica aberta para sempre, sem baixar estoque e **sem erro nenhum**.

#### ⚠ Perda de rastreabilidade — consciente, e é preciso que esteja escrita

O contrato do `SaveReqMat` (22 campos de cabeçalho, §10.16) **não tem `CodigoUsuario` nem `UsuarioLogado`**. O `PedComp` tem os dois — e foi exatamente mandando o login no corpo que o L7-B de Projetos resolveu o achado A-8 (dois pedidos foram parar no ERP carimbados com o Pedro).

Aqui não há campo onde mandar. `op_reqmat.codigo_usuario` está preenchido em **287/287** e vem da **sessão autenticada no Alvo**, não do payload — a RM `0000002271`, criada por API em 05/08, saiu como `PEDRO.SCRIGNOLI`. Como o gateway autentica por **conta de serviço** (env do Render), **toda RM criada pelo Hub gravará a conta de serviço na coluna "Usuário" do Alvo.**

A rastreabilidade real fica em duas pernas, e as duas são firmes:
- **`CodigoFuncionario`** no ERP — fixo, do perfil de quem cria (decisão (c) de 07/08);
- **`op_requisicoes.criado_por`** no livro do Hub, com o `user_id` real.

⇒ **Isto é esperado, não é bug.** Quem abrir a coluna "Usuário" do Alvo daqui a seis meses e concluir que "a conta de serviço requisitou tudo" está lendo um campo que o nosso caminho não controla. Mudar exigiria autenticação por usuário no gateway — outra frente, e ela colide com o BL-26 (credencial do Alvo no localStorage), que é justamente o padrão que estamos evitando.

#### O que ficou de fora, de propósito

**Atendimento.** `ValidarAtendimento` e `FinalizarAtendimento` **não** estão na whitelist, e o BL-21 (endpoint de lotes disponíveis) segue aberto. A criação não depende disso.

---

> **Nota de ordem (07/08/2026).** As quatro subseções abaixo (10.25 a 10.28) cobrem o ciclo de 06–07/08 e são **anteriores em tempo** à §10.24, que foi escrita antes delas. A ordem de leitura recomendada é: **10.25 (RBAC) → 10.26 (OP-2.4) → 10.27 (OP-2.5) → 10.28 (OP-2.6) → 10.24 (OP-2.7)**. A numeração não foi refeita de propósito: `sql/OP-2.7.sql` e a §2 já referenciam "§10.24", e renumerar quebraria ponteiros gravados fora deste arquivo.

### 10.25 — RBAC do módulo: o que estava quebrado e a segregação de funções (06/08/2026)

> 🔴 **O ACHADO QUE EXPLICAVA AS 2 OPs.** A Fase 1 foi entregue, validada e publicada em 23/07 — e ficou **invisível** por duas semanas. Duas causas somadas: **ninguém tinha `operador_producao` nem `gestor_producao`** (zero atribuições em 52 usuários ativos) e **`producao.access` era órfã do papel `admin`**. ⇒ Só quem tem `profiles.is_admin = true` enxergava o módulo — **uma pessoa**.
>
> **Não era falta de adesão do chão de fábrica: era falta de atribuição de papel.** O módulo estava construído e ninguém conseguia abrir a tela. Mesmo padrão em NF-e: `analista_fiscal` existe, concede `compras.nfe.lancar`, e tem **zero** usuários.

**Corrigido em 06/08/2026:**

1. **As 13 permissões órfãs mapeadas ao papel `admin`** — o papel concedia **42 de 55**; as 13 fora eram as 4 de `cartao`, as 3 de `producao`, 5 de `compras` e `intercompany.master.download_pdf`, **todas criadas DEPOIS do papel**. Agora são 55.
2. **Três permissões novas** — `producao.rm.access`, `producao.rm.create`, `producao.rm.atender`, **já mapeadas ao `admin` na criação**, para não repetir o defeito.
3. **Três papéis, com segregação de funções PROVADA** (validada com `user_has_permission`, não por inspeção do cadastro):

| Papel | Cria OP | Gere OP | Vê RM | Cria RM | Atende RM |
|---|---|---|---|---|---|
| `gestor_producao` | ✅ | ✅ | ✅ | ✅ | — |
| `operador_producao` | ✅ | — | ✅ | ✅ | — |
| `almoxarifado` *(novo)* | — | — | ✅ | — | ✅ |

⇒ **Quem requisita não atende, e quem atende não requisita.** Não é preferência de desenho: é o controle que faz a baixa de estoque ter dois pares de olhos.

4. **Papéis atribuídos:** `operador_producao` para **maria.santos@** (funcionário `0000098` = **Maria Eduarda Santos Araujo**, a autora de 246 das 280 RMs de produção do ano) e **ryan.santos@** (`0000063`).

**Três armadilhas encontradas no caminho — todas silenciosas:**

- ⚠ **Pedro é o único `is_admin` de 52.** Ele vê tudo por bypass, então **erro de permissão nunca aparece para ele**. Toda tela nova precisa de teste com usuário **sem** a flag — foi o que a OP-1.6 fez com `nfe@pfbrazil.com`, e é o motivo de aquilo ter valido a pena.
- ⚠ **Pegadinha de JOIN:** `hub_user_roles.user_id` casa com **`profiles.user_id`**, NÃO com `profiles.id`. A consulta errada devolve "zero papéis" e faz o RBAC inteiro parecer não usado — conclusão errada a partir de dado certo.
- ⚠ **Vínculo trocado, encontrado e corrigido:** o perfil do Pedro apontava para **Adna Ferreira (`0000001`)** em vez de **Pedro Scrignoli (`0000149`)**. Falha 100% silenciosa: nenhum erro, nenhuma tela quebrada — só um documento saindo do ERP com o nome errado. `nfe@pfbrazil.com` compartilha o `0000149` de propósito (conta de serviço).

### 10.26 — OP-2.4 · a tela de RM, e três medições que mudaram o código (06/08/2026)

Fila, detalhe por número e consolidado de material na tela da OP. O que vale registrar não é a tela — é que **três coisas foram medidas ANTES de codificar, e as três mudaram a implementação**:

1. **O aviso de divergência de lote quase virou ruído.** A ideia inicial era avisar sempre que a soma dos lotes não batesse com o atendido. Medindo: **597 itens** não têm linha de lote — porque o produto tem `ControlaLote = "Não"`, não porque falta dado. O aviso teria disparado em quase todos. ⇒ Só dispara quando **existe** linha de lote e ela diverge: **4 casos reais** (§10.20).
2. **O filtro "com saldo em aberto" mede nos ITENS**, com `quantidade_saldo > 0` — não infere do status do cabeçalho. Status é resumo; saldo é fato, e os dois divergem quando o atendimento é parcial.
3. **Os três campos da §10.4 blindados com comentário do porquê** — `quantidade_saldo` espelhado (nunca `pedida − atendida`), `quantidade_atendida_maior` carimbado pelo Alvo, e lotes somados por `quantidade` (nunca `quantidade_bruta`). Sem o comentário, o próximo a passar "conserta" para a subtração óbvia e quebra a conta em silêncio.

### 10.27 — OP-2.5 · `ausente_desde` visível, e a fila que não era platô (06/08/2026)

**(a) O bug era de TELA, não de dado.** A RM `0000002271`, excluída no Alvo, aparecia viva na fila — o espelho **sabia** (`ausente_desde` carimbado em 05/08 19:16); quem não mostrava era a tela. `getStatusRM` passou a receber a **linha** (molde do `statusPedido.ts`), com `ausente_alvo` como **primeira condição da precedência**, rótulo "Excluída no ERP" e `line-through`.

⚠ **E o filtro teve de acompanhar:** os status do Alvo passaram a exigir `.is("ausente_desde", null)` no server-side. Sem isso, badge e filtro divergem — a RM apareceria riscada em "Atendida Total" e contaria no chip daquele status.

**(b) 🔴 Inanição por prioridade — não era platô de convergência, era fome.** A fila do passo B ordena por `codigo_tipo_req_mat asc NULLS LAST`, e `'0000002'` (produção) é o MENOR dos quatro códigos. RM não-terminal **nunca sai da fila** (é desenho: `precisa_releitura` não zera). Com **~120 RMs de produção permanentemente na fila** e `LOAD_BATCH = 60`, a conta era `120 ≥ 60` **em toda execução** — o lote inteiro ia para a produção e as ~400 de outros tipos **nunca chegavam ao topo**.

Visível em `sync_runs`: `fila_restante` 646 → 621 → 589 → 549 → 520 → **520**. Estacionou.

Teto para **150** ⇒ a fila **caiu pela primeira vez**: **520 → 501 → 475 → 456 → 434 → 409 → 382**.

**(c) Cron agendado, e o horário mudou por um motivo medido.** `sync-reqmat-4x-dia`, movido de `:15` para **`:25`** porque o `sync-docfin-3x-dia` roda `:10` e pode chegar a **110 s** — 5 minutos de folga não bastam. Provado em campo: execuções `pg_cron` em **53–56 s**, zero erros. ⚠ **A folga contra o watchdog de 110 s é de ~2×, não os ~4× estimados** no comentário da Edge Function. Vigiar.

**(d) "Conferir no ERP" — refresh só-leitura.** Chama `ReqMat/Load` pelo passthrough com o JWT do usuário, compara **em memória** e **não persiste nada**. Foi a terceira opção, e a certa: abrir a RPC a `authenticated` seria a classe do BL-5, e criar uma Edge nova seria infraestrutura para um botão.

⚠ **Não marca ausência.** Um 412 isolado **não é prova** de exclusão — é a lição do módulo de Suprimentos, cuja primeira versão marcava exclusão em qualquer 404 e **marcou 7 pedidos VIVOS**. Quem confirma ausência é o `sync-reqmat`, que tem cross-check (listagem completa, zero erros, sem truncação, teto de 5% do ano).

⚠ **412, não 404 — e as duas rotas do MESMO gateway divergem.** O Alvo devolve `412 Precondition Failed` para registro excluído; o `/alvo/passthrough` **repassa o 412 puro**, enquanto a rota `/ped-comp` **converte em 404** por regex na `Message`. ⇒ **Ninguém deve assumir o código de status pela entidade, e sim pela rota que falou.**

### 10.28 — OP-2.6 · o cron da RM em Ferramentas, sem RPC nova (07/08/2026)

> 🔴 **NÃO FOI PRECISO CRIAR RPC INTERMEDIÁRIA** — e é o achado que vale além desta tarefa.
>
> `call_sync_reqmat_cron` é a única das nove `call_sync_*_cron` **sem EXECUTE para `anon`/`authenticated`** (nasceu com REVOKE na OP-2.3, de propósito). Parecia que o botão "Rodar Agora" exigiria uma RPC nova para contornar isso. Não exige: **`sync_cron_trigger_now` é `SECURITY DEFINER` com owner `postgres`**, então a chamada interna roda com os privilégios do owner e **o REVOKE não a alcança**. O usuário continua sem poder chamar o disparador diretamente — que é exatamente o desejado.

⇒ **Duas linhas de SQL** (`_sync_cron_resolve` + `sync_cron_trigger_now`), e `sync_cron_get_status` / `list_runs` / `pause` / `resume` passaram a servir a RM **de graça** — todas resolvem o job pela mesma função. Gate: **`ferramentas.cron.view`**, sozinho.

⚠ **O `CREATE OR REPLACE` não preserva `SECURITY DEFINER` nem `SET search_path`** — por isso as duas funções foram reescritas INTEIRAS. Um replace que esquecesse as cláusulas transformaria o `trigger_now` em SECURITY INVOKER, e ele perderia o owner (⇒ deixaria de conseguir chamar as `call_sync_*`) e a visibilidade do `vault`.

⚠ **O modal NÃO podia ser copiado.** As cinco telas antigas renderizam `sync_runs.detalhes` como lista de MUDANÇAS (`MudancaRow`, `{tipo:'req'|'ped', numero, status_novo}`) e usam a presença da chave `erro` para pintar de vermelho. O `sync-reqmat` grava outra coisa: `{etapa:'lista'|'gravacao'|'load'|'ausencia'|'watchdog'|'exception', info?, erro?}`. Copiado sem tocar **não quebra — MENTE**: badge "Ped" com número vazio, e **a linha de ausência caindo no bloco vermelho de erro**, porque ela usa a chave `erro` para dar **destaque**, não para sinalizar falha. ⇒ `EtapaRow` próprio, dirigido pela chave `etapa`, com ausência e watchdog em **âmbar**.

⚠ **Dois defeitos no `sql/OP-2.5.sql` já aplicado** (bloco imutável — não foi alterado; ambos registrados no cabeçalho do `sql/OP-2.6.sql`):
1. o rollback sugere `sync_cron_pause(p_job => 'sync-reqmat')`, mas `p_job` é o **alias**, não o `job_name` — a linha falha com "Job desconhecido". Depois da OP-2.6 a forma correta passa a existir: `'reqmat'`;
2. o arquivo diz `'15 12,15,18,21 * * 1-5'` onde o banco tem **`:25`** — as duas pontas do banco estão em sincronia; quem diverge é o arquivo no repo.

### 10.29 — OP-2.8 · a RM aparece na hora, e a lição de EIXO que a produziu (08/08/2026)

**O PRIMEIRO TESTE REAL DA CRIAÇÃO — 08/08/2026.** A RM `0000002286` percorreu o ciclo de três passos em **10 segundos**: Insert **00:08:45** → Update **00:08:53** → Load **00:08:55**. Conferida no Alvo:

| Campo | Valor | Por que importa |
|---|---|---|
| `Origem` | **`"Importação"`** | 🟢 **A primeira RM da história do Hub que nasce distinguível das criadas na tela** — as 685 anteriores são `ManualAlvo`. A métrica de vazamento da §10.7 nasceu funcionando. |
| `TipoAtendimento` | **`"Manual"`** | Nasceu **viva**. As 16 `Automático` do ano estão **todas abertas** — o passo 2 pegou. |
| `CodigoFuncionario` | `"0000149"` | O do perfil de quem criou, como desenhado. |
| `Descricao` | `"OP 2026-0503 - teste"` | O prefixo da OP dentro do limite de 40 do Alvo. |

O ciclo, portanto, estava certo. O que ele revelou foi outra coisa.

> 🔴 **O DEFEITO NÃO ERA DE FILTRO, ERA DE EIXO — e é a terceira ocorrência da mesma família.**
>
> O bloco "Aguardando sincronização" da OP-2.7 perguntava **`status_envio IN ('pendente','enviado')`**. Parecia certo e não era:
>
> - **`status_envio`** responde *"o ERP aceitou?"* — é o estado do **ciclo de envio**;
> - **presença em `op_reqmat`** responde *"o Hub já enxerga?"* — é o estado do **espelho**.
>
> O bloco perguntava a primeira quando precisava da segunda. `confirmado` significa que o **Alvo** aceitou, não que o **espelho** viu.
>
> Medido na hora: `status_envio = confirmado` / **`ja_no_espelho = false`**.
>
> ⇒ **Onde há livro e espelho, todo filtro de UI declara qual dos dois está perguntando.** Mesma família da **§10.27(a)** (`ausente_desde`: o espelho sabia, a tela não perguntava) e da **§6.3-N** (`item.BaixaEstoque` = regra × `cabecalho.BaixouEstoque` = fato). Três vezes o mesmo formato: dois eixos, nomes parecidos, um lido pelo outro, erro silencioso e plausível.
>
> ⇒ E o motivo de só aparecer agora: **um caminho feliz que nunca rodou não é caminho validado.** Nenhum teste anterior tinha chegado ao estado terminal `confirmado` — a RM `0000002286` foi a primeira, e sumiu das duas vistas ao mesmo tempo.

**A decisão.** A RM criada pelo Hub é gravada no espelho **no ato da criação**. Uma tabela só — sem merge em memória, sem view, sem bloco separado. O sync assume na passada seguinte e atualiza o que só ele sabe: atendimento, lote, saldo, status.

**🔴 Semear do LOAD, nunca do payload — e a razão é medida, não estética.** Na `0000002286` o Hub enviou `Sequencia: 1` no Insert e o `ReqMat/Load` devolveu **`Sequencia: 2`**: **o Alvo renumera**. Como a sequência é parte da PK do item e é o que `op_reqmat_lotes.sequencia_item` referencia, semear do payload gravaria PK errada hoje e produziria **lotes órfãos da FK** amanhã, no atendimento. O Load também é quem traz `Data`, `Status`, `Operacao`, `DataValidade` — tudo preenchido pelo servidor.

**Um mapper, dois mundos — e G1 foi escolhido por medição.** O mapper saiu do `sync-reqmat/index.ts` para **`supabase/functions/_shared/reqmatMapper.ts`**, importado pela Edge Function (Deno) **e** pelo frontend (Vite). A alternativa era duplicá-lo com um teste de contrato; foi descartada porque duas cópias vivas de 100+ linhas divergem meses depois, em silêncio, e o teste só pega o que alguém lembrou de cobrir. **Medido antes de decidir:** `tsc --noEmit -p tsconfig.app.json` e `vite build` aceitam o import de fora de `src/` sem configuração nova, e o mapper compila sozinho, com resolução completa, sem erro. ⚠ A condição de o arranjo continuar valendo é o arquivo **não ter dependência nenhuma** — nada de `Deno.*`, nada de import de URL, nada de `@/`.

**🟢 O contrato das duas rotas é o mesmo — medido, não suposto.** Comparei chave a chave o `ReqMat/Load` do navegador (JWT do usuário, gravado em `op_requisicoes.resposta_alvo` da 2286) contra o `raw` do sync (`X-System-Secret`): **103 vs 97 chaves, e as 6 de diferença são exatamente os 6 arrays** que o sync remove ao montar o `raw` (`ItemReqMatChildList`, `ReqMatArqAnexoChildList`, `ReqMatClasseRecDespChildList`, `ReqMatHistDistrSeparadorChildList`, `InformationResult`, `ListaMensagens`). **Zero chave escalar divergente, nos dois sentidos.** A credencial não muda o corpo.

**O caminho de escrita.** `op_reqmat_semear_criacao` (`sql/OP-2.9.sql`), `SECURITY DEFINER`, com **duas** trancas: o gate `producao.rm.create` e a exigência de que o número exista em `op_requisicoes` com `status_envio = 'enviado'` — a janela exata entre o passo 1 e o passo 4. Não é RPC genérica de escrita no espelho. Ela insere **só a chave do cabeçalho** e delega itens, lotes e carimbos à `op_reqmat_aplicar_load`, preservando a transação única daquela função (a janela sem filhos existe por um motivo medido: o consolidado da OP leria "atendido 0"). A chamada atravessa o REVOKE pelo **mecanismo do §10.28** — a chamadora é DEFINER com owner `postgres`, dono da função de destino. Conferido no banco: `postgres` pode, `authenticated` não.

⚠ **Nenhuma policy de INSERT em `op_reqmat`**, e a medição explica por quê: os GRANTs de tabela são os amplos do Supabase (`authenticated` tem INSERT/UPDATE/DELETE nas três tabelas do espelho) ⇒ **quem barra a escrita é a AUSÊNCIA de policy, não o grant.** Criar uma abriria o espelho ao console do navegador de qualquer um dos 100+ usuários — BL-5 literal.

**🔴 A guarda de ausência, que veio no mesmo pacote de propósito.** Até aqui, toda linha do espelho tinha nascido da listagem, e "está no espelho e não na listagem" só podia significar exclusão no ERP. Com a semeadura isso deixou de valer: passa a existir linha que a listagem legitimamente ainda não conhece — tirada antes, ou servida de cache (`DisabledCache: false`, e é assim de propósito). **O teto de 5% não protege**: uma RM sozinha passa folgada (limite ~34 num espelho de 688). O sintoma seria a RM recém-criada **riscada como "Excluída no ERP"** por até 3 h — a mentira exata que a OP-2.5 consertou —, e ela **se corrigiria sozinha** na passada seguinte, o que é pior, não melhor: erro que se conserta sozinho é erro que ninguém investiga.

⇒ `marcarAusencias` passou a receber o instante em que o **retrato da listagem** foi tirado (carimbado *antes* da chamada ao Alvo, para proteger também o que for criado durante ela) e a **não avaliar** linha com `created_at` posterior. Vale para qualquer origem futura, não só para a criação pelo Hub. As poupadas entram em `sync_runs.detalhes` como **info** — um número que crescesse sem parar significaria que a listagem nunca alcança o que o Hub grava, e isso, sim, seria defeito.

⚠ **Exige REDEPLOY.** Enquanto ele não sair, a função em produção é a antiga e não conhece a guarda. **Ordem obrigatória: SQL → redeploy → Publish.**

**O indicador na linha, sem DDL.** A linha semeada tem `detalhes_carregados_em` preenchido e atendimento zerado — a olho, indistinguível de uma RM do Hub sincronizada há dias (`Origem = Importação` e `status = Aberta` valem nos dois casos). O sinal é derivado: **`op_requisicoes.confirmado_em` posterior ao `op_reqmat.sincronizado_em`**. O primeiro passo A do sync reescreve `sincronizado_em`, a desigualdade se inverte e **o aviso se apaga sozinho** — nada a limpar, nada a expirar. Foi o mesmo critério que matou a versão "avisar sempre" da §10.26-1: aviso que não se resolve vira ruído, e ruído deixa de ser lido.

**🔴 Por que `revoke ... from public` não basta neste projeto — causa encontrada.** Na OP-2.7 o arquivo trazia `revoke execute … from public` e as três `op_rm_*` nasceram **mesmo assim** chamáveis por `anon`. A causa está em `pg_default_acl`, lida no banco em 08/08/2026: há um `ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT EXECUTE ON FUNCTIONS TO anon, authenticated, service_role` (padrão do Supabase). ⇒ Toda função nova criada por `postgres` em `public` nasce com EXECUTE concedido **nominalmente a `anon`**, não herdado de PUBLIC — e `revoke from public` não o alcança. **A única forma de tirar é `revoke execute … from anon`, com a assinatura completa.** É regra para TODA RPC nova deste projeto, não exceção desta.

**O que saiu da tela.** O bloco "Aguardando sincronização" (azul, informativo) virou **"Envios incompletos"** (âmbar, acionável), e em regime normal fica **vazio e invisível** — essa é a diferença que importa: o que aparecer ali é exceção e pede ação. O que ele cobre agora: `enviado` não confirmado (a RM existe no ERP em `Automático`, ou seja morta — único caso em vermelho, com "Concluir envio (passo 2)") e `confirmado` sem espelho (a semeadura falhou; só aguardar o sync). `erro` ficou **de fora**: ali nada foi criado no ERP, não há divergência livro×espelho a resolver, e mantê-lo encheria o bloco de lixo permanente.

**Fila do sync — conferido, não deduzido.** A linha semeada nasce `precisa_releitura = true` (a expressão real no banco tem um termo a mais do que o registrado até aqui: `status IS NULL`), então entra na fila naturalmente. ⚠ Mas entra **no fim**: a ordem é `codigo_tipo_req_mat asc` → `detalhes_carregados_em asc`, e a semeadura carimba `now()`. Hoje não morde — fila de produção **123** contra `LOAD_BATCH = 150`, folga de 27. Se a fila de produção passar de 150, a RM criada pelo Hub é a última a ser relida. **Número a vigiar.**

#### ✅ Validação com usuário NÃO-admin (08/08/2026) — a peça que faltava desde 06/08

A RM **`0000002287`** foi criada por **`pscrignoli@gmail.com`**: `is_admin = **false**`, papel `operador_producao`, funcionário `0000001` (**Adna Ferreira**). Resultado medido:

- **semeada no espelho no mesmo instante do `confirmado_em` — 01:16:07.** Não "poucos segundos depois": o mesmo carimbo. É a prova de que a semeadura acontece dentro do ciclo, e não por um sync que passou por acaso;
- `origem = Importação`, `tipo_atendimento = Manual`;
- **`ausente_desde` NULL** — a guarda de recência segurou. Sem ela, esta era a RM que apareceria riscada como "Excluída no ERP";
- `codigo_funcionario = 0000001`.

⇒ **A cadeia inteira foi validada sem bypass**, e são quatro gates distintos em série: `producao.rm.access` (entrar na tela) → `producao.rm.create` (ver o botão) → `op_rm_criar` (gate próprio da RPC) → `op_reqmat_semear_criacao` (gate próprio da semeadura). Nenhum deles tinha sido exercitado por quem não é admin.

> 🔴 **Por que isto era a peça que faltava.** Em 06/08 mediu-se que **o Pedro é o único `is_admin` de 52 usuários** (§10.25) — e admin tem bypass, então **erro de permissão nunca aparece para ele**. Toda a cadeia de gates do módulo era, até aqui, código não exercitado. É a mesma lição que a OP-1.6 pagou com `nfe@pfbrazil.com`, e o motivo de aquilo ter valido a pena.

⇒ **E prova o desenho do funcionário fixo (decisão (c) de 07/08).** Duas contas criaram RM no mesmo dia, com **funcionários distintos** — `0000149` na `2286` e `0000001` na `2287` —, cada uma com o seu. O código não chumba funcionário: ele **sai do perfil de quem cria**. Era a afirmação central da §10.24 e agora tem evidência de campo, não só de leitura.
### 10.30 — Atendimento pelo Hub: o endpoint de lotes e a decisão de processo (08/08/2026)

> Esta subseção fecha o **BL-21** e registra a decisão que condiciona o desenho da Fase 4. O plano
> executável da fase é o **`PLANO-RM-ATENDIMENTO.md`** (tarefas AT-1 a AT-6).

#### O endpoint de lotes — capturado

```
POST https://pef.it4you.inf.br/api/CtrlLoteLocArmaz/ListaCtrlLoteLocArmaz
```

⚠ **Não está na whitelist do `erp-proxy`** — hoje responde 403 no passthrough. Entra na AT-1,
junto com `ReqMat/ValidarAtendimento` e `ReqMat/FinalizarAtendimento`.
...
O `ClassInstance` carrega `ControlaLote`, `CodigoProduto`, `CodigoLocArmaz`, `CodigoProdUnidMed` e
`PosicaoProdUnidMed` — é por eles que o Alvo resolve a lista.

**Resposta:** `{ "ListaCtrlLoteLocArmaz": [ … ] }`. Campos que importam: `NumeroCtrlLote` ·
`DataValidadeCtrlLote` · **`DataFabricacao`** (que a tela do Alvo nem mostra) ·
`QuantidadeSaldoProdUnidMedPrincipal`.

Espécime real (`001.003.00059`, local `001`, 08/08/2026):
...
🟢 **A lista vem ordenada por validade** — o Alvo já entrega em ordem FEFO. Isso torna barata a
sugestão automática que a §10.20 registrou como oportunidade (lá o FEFO é manual, escolhido por
quem atende).

⚠ **`QuantidadeReservaLote` e `QuantidadeEmpenhoLote` vêm nulos** ⇒ o saldo é **bruto**, não
descontado de reserva nem de empenho. Hoje não morde (`GeraEmpenho` é sempre `"Não"` nas RMs de
produção, §6.2/Resolvidas), mas **o número não é "disponível", é "saldo"**. Se o empenho um dia
for usado, esta leitura passa a mentir.

⚠ `Deletar: false` e `Mensagem: null` na resposta sugerem que o mesmo objeto volta ao Alvo com a
seleção — provavelmente o que faz o **`RelacionarCtrlLoteLocArmaz`** (5,2 kB no Network, entre a
lista e o `ValidarAtendimento`). **Não capturado** — registrado como **AT-2**. O atendimento por
API da RM `0000002273` (§10.19) funcionou **sem** ele, então ou é conveniência da tela, ou faz
algo a mais que não notamos.

#### A decisão de processo: Hub **ou** Alvo

**Decisão do Pedro (08/08/2026):** o almoxarifado **pode atender no Hub ou no Alvo** — as duas
portas ficam abertas. Isso é revisão consciente da §6.1-1, que decidira "o almoxarifado atende no
Alvo": a §10.19 provou que pelo Hub é tecnicamente possível, então virou **escolha de processo**.

🔴 **A consequência que o desenho tem de tratar.** Duas pessoas — ou a mesma, em duas abas —
atendendo a mesma RM em telas diferentes. O Hub monta o objeto a partir de uma leitura de segundos
atrás; se alguém atendeu no Alvo nesse meio-tempo, o Hub manda quantidade já consumida. **E o Alvo
aceita atendimento acima do pedido** — é exatamente assim que o `QuantidadeAtendidaMaior` nasce
(§10.4) — então **não haveria erro: haveria baixa em dobro.** É o BL-18 chegando por outra porta.

⇒ **Releitura obrigatória imediatamente antes de finalizar**, com recusa se as quantidades mudaram
desde a abertura da tela. **Requisito, não refinamento.**

⚠ **Risco de adoção, registrado.** Se o Hub é opcional, na prática ninguém migra — a tela do Alvo
é a que a pessoa conhece. É o mesmo mecanismo que deixou a Fase 1 invisível por semanas (§10.25),
por outro motivo. **Recomendação:** construir como se fosse o caminho único, manter as duas portas
em convivência **com data para fechar a do Alvo**; sem data, não fecha.

#### Decisão adjacente: o Recebimento não muda

**A inspeção de recebimento continua sendo feita no Alvo, pela Qualidade.** A tela
`/recebimento/fila` (§9.2) permanece **consulta admin-only** — o gate é `_is_admin()` na RLS de
`rec_laudos`, sem permissão nomeada, e hoje **só o Pedro a vê** (único `is_admin` de 52).

⚠ **O papel `almoxarifado` NÃO dá acesso ao Recebimento.** Ele concede só `producao.access`,
`producao.rm.access` e `producao.rm.atender` — é do módulo Produção. Quem receber esse papel verá
a fila de RM, não a Fila de Inspeção. Registrado para não haver surpresa.

---

### 10.31 — AT-1 e AT-2 fechados: o fluxo de atendimento tem mapa completo (09/08/2026)

> Fecha a **AT-1** (whitelist) e a **AT-2** (`RelacionarCtrlLoteLocArmaz`). Com isto, **a Fase 4
> não tem mais incógnita técnica** — o que resta é construção.

#### AT-1 — whitelist liberada e provada

Três entradas novas em `erp-proxy/src/routes/alvo.ts`, deploy verde no Render:

```ts
  // ─── Atendimento de RM (Fase 4 do módulo OP, AT-1) ─────────────────────────
  // ⚠ ESCRITA que BAIXA ESTOQUE. O passthrough valida só JWT, sem gate de papel
  //    (BL-5) — remover se a Fase 4 for interrompida.
  "CtrlLoteLocArmaz/ListaCtrlLoteLocArmaz",  // lotes disponíveis por produto/local (POST)
  "ReqMat/ValidarAtendimento",               // passo 1 do atendimento
  "ReqMat/FinalizarAtendimento",             // passo 2 — baixa estoque
```

Provado no console (403 → outro status nos três):

| Endpoint | Antes | Depois |
|---|---|---|
| `CtrlLoteLocArmaz/ListaCtrlLoteLocArmaz` | 403 | **417** |
| `ReqMat/ValidarAtendimento` | 403 | **200** |
| `ReqMat/FinalizarAtendimento` | 403 | **200** |

🔴 **ARMADILHA NOVA — `FinalizarAtendimento` com payload vazio responde 200, não erro.** Devolve
um objeto ReqMat **em branco** (`Numero: ""`, `CodigoEmpresaFilial: ""`, `Status: "Aberta"`,
`BaixouEstoque: "Não"`) em vez de recusar. ⇒ **O Hub não pode tratar 200 como sucesso.** A guarda
é a mesma do `analisarRespostaReqMat` (§10.19): exigir a **âncora** — `ReqMat.Numero` presente,
não-nulo e não-vazio — antes de aceitar a resposta. Nada foi criado no teste; o objeto é eco.

#### O contrato dos dois endpoints de lote

Ambos em `POST https://pef.it4you.inf.br/api/CtrlLoteLocArmaz/…`, pelo passthrough.

**Cabeçalhos** (o passthrough resolve; registrado para o caso de chamada direta):
`Content-Type: application/json` · `Riosoft-Token: <token>` · `LocalConnection` ·
`X-Requested-With` · `Authorization`. O CORS do Alvo expõe `Riosoft-Token`, `TotalCount`,
`modulename` e `exceptionMessage`.

##### 1. `ListaCtrlLoteLocArmaz` — quais lotes existem
...
{
  "Origem": 7,
  "Data": "2026-08-09T20:12:11.284-03:00",
  "DataMovimentacao": "2026-08-09T20:12:11.284-03:00",
  "EspecieDocumento": "RM",
  "NumeroDocumento": "0000002277",
  "SerieDocumento": "0",
  "SequenciaDocumento": null,
  "OperacaoLote": "Saída",
  "OperacaoRM": "Retirada",
  "CodigoTipoLanc": "",
  "ClassInstance": { "…o item completo, como vem do ReqMat/Load…" }
}
```

⚠ **`Origem: 7`** é constante mágica — não se sabe o que significa, mas é obrigatória.
⚠ O `ClassInstance` é o **item**, não o cabeçalho. É dele que o Alvo tira `CodigoProduto`,
`CodigoLocArmaz`, `CodigoProdUnidMed`, `PosicaoProdUnidMed` e `ControlaLote`.

**Response:** `{ "ListaCtrlLoteLocArmaz": [ … ] }`, **ordenada por validade** (FEFO nativo).
...
| `QuantidadeBruta` | ⚠ ver abaixo — é a quantidade **original** do lote |
| `QuantidadeReservaLote` · `QuantidadeEmpenhoLote` | **nulos** ⇒ saldo é bruto |

##### 2. `RelacionarCtrlLoteLocArmaz` — 🟢 o alocador FEFO do servidor

**Este é o achado da AT-2.** Não é conveniência da tela: **é o Alvo alocando.**

**Request:** o mesmo envelope da lista, com dois acréscimos —
`Lista` (o array devolvido pelo `ListaCtrlLoteLocArmaz`) e o `ClassInstance` com a
**quantidade a atender** já preenchida.

**Response:** o **item de volta**, com a `CtrlLoteItemReqMatChildList` **já montada**. No
espécime (RM `0000002277`, item `001.003.00047`, 20 a atender): uma linha, lote `0002467`,
quantidade 20 — **o de validade mais próxima**. O usuário não alocou nada.

⇒ **O Hub NÃO precisa implementar FEFO.** Chama o `Relacionar` e recebe a alocação pronta. E
continua podendo montar à mão quando quiser — a RM `0000002273` (§10.19) foi atendida com rateio
manual entre dois lotes, sem passar por ele.

⇒ **Isto explica um episódio de 05/08:** no teste da RM `2272`, a quantidade do modal de lote
vinha **pré-preenchida** e foi preciso ajustá-la. Era este endpoint que a havia preenchido.

#### 🟢 O mistério do `QuantidadeBruta` — resolvido

A §10.20 registrou o campo como "significado não fechado", porque os números não fechavam:
5 para 1 galão numa RM, 4 para 4 galões noutra. **É a quantidade ORIGINAL do lote**, não o total
do atendimento:

| RM | Lote | Saldo hoje | `QuantidadeBruta` | Bate? |
|---|---|---|---|---|
| `2275` | `0002696` | 5 | 5 | ✅ lote inteiro ainda disponível |
| `2272` | `0002311` / `0002312` | 1 e 4 | 4 e 4 | ✅ tamanhos originais |
| `2277` | `0002467` | 154 | 350 | ✅ entrou com 350, restam 154 |

⇒ **A regra prática não muda:** o quanto sai de cada lote é `QuantidadeProdUnidMedPrincipal`.
Mas agora o `QuantidadeBruta` tem uso legítimo — dá para mostrar "154 de 350" na tela, que é
informação de estoque que o Alvo não exibe.
...
```json
{ "CodigoClasseRecDesp": "11.03", "Percentual": 100, "CodigoEmpresaFilial": "1.01" }
```

**Não aparecia em nenhum `ReqMat/Load` anterior** — na `2251` vinha vazia. É rateio por classe de
receita/despesa, e é **assunto de Controladoria**: se a RM criada pelo Hub nascer sem isso, pode
ficar sem classificação contábil. ⇒ **Verificar se a classe vem do produto, do centro de custo ou
do tipo de RM**, e se as RMs criadas pelo Hub em 08/08 (`2286`, `2287`) a têm.

#### Confirmação de que nada foi atendido nesta captura

O log do Network mostra a sequência `ListaCtrlLoteLocArmaz` → `RelacionarCtrlLoteLocArmaz` →
`ExpireSession` e, depois, só `LoadParentName` e `GetNotificationUnreadCount`. **Nenhum
`ValidarAtendimento`, nenhum `FinalizarAtendimento`** — a tela foi fechada no X.
⇒ Protocolo que evita repetir o **BL-29**: capturar e **fechar no X**, nunca concluir o fluxo.

#### O que sobrou no log e NÃO é preciso

`GetMetadataConfiguration` · `LoadParentName` · `Exist?shortcut=rm&className=ctrlLoteLocArmaz` ·
`ExpireSession` — infraestrutura da tela do Alvo. O Hub tem tela própria. O `Exist?` parece
verificar "este item tem lote a selecionar?", e é redundante: o `ControlaLote` já vem no item.

⚠ **Ficam por capturar, para a Fase 5:** os botões **Reservas**, **Empenhos** e **Estoque
Transitório** da tela de saldo. O Estoque Transitório é o **BL-10** — candidato a onde vivem as
1.433 un de pericárdio retidas (§6.3).
...
> ⚠ **HISTÓRICO — superado. O ponto de retomada vigente é a §11.2.** Os quatro passos abaixo estão **todos FEITOS** (o nº 4, "alguém precisa abrir OP real", caiu em 06–07/08: existem duas OPs `ABERTA`). Mantido na íntegra pela disciplina da §6.3-N.
...
> ⚠ **HISTÓRICO — superado pela §11.2.** As quatro pendências desta subseção foram cumpridas em 06–07/08 (`sql/OP-2.4.sql` aplicado, redeploy e `sql/OP-2.5.sql` feitos, tela validada e publicada). O que restou dela — a conversa com a Maria Eduarda sobre o desenho — virou o passo 1 da §11.2, agora como **teste real de criação com o usuário dela**. Mantido na íntegra pela disciplina da §6.3-N.
...
0. **(OP-2.5) Redeploy do `sync-reqmat`** — `supabase functions deploy sync-reqmat --project-ref hbtggrbauguukewiknew` — e **só depois** aplicar `sql/OP-2.5.sql` (agendamento). Invertido, o cron roda com `LOAD_BATCH=60` e a inanição continua. Conferir na 1ª execução automática: `triggered_by='pg_cron'`, `detalhados=150` (se vier 60, o redeploy não saiu) e `fila_restante` **voltando a cair** de 520.
...
## 11.2 — Ponto de retomada — 07/08/2026

> ⚠ **HISTÓRICO — superado pela §11.3.** O item 1 ("o primeiro teste real da criação") **caiu em 08/08/2026**: a RM `0000002286` foi criada pela tela e o ciclo de três passos passou em 10 s. Ela não apareceu na fila, e é isso que a OP-2.8 resolve (§10.29). Os itens 2 a 5 continuam válidos e foram levados para a §11.3. Mantido na íntegra pela disciplina da §6.3-N.

> ⚠ **Esta subseção substitui os planos das §11 e §11.1 acima**, mantidos como histórico (disciplina da §6.3-N: nada é apagado). **Os quatro passos da §11 e as quatro pendências da §11.1 estão FEITOS** — permissões mapeadas, papéis criados e atribuídos, `sql/OP-2.4.sql` e `sql/OP-2.5.sql` aplicados, redeploy do `sync-reqmat` feito, cron agendado e provado, e as duas OPs abertas existem. O item "**Alguém precisa abrir OP real no Hub**" também caiu: há **duas** OPs `ABERTA` (`2026-0501` e `2026-0503`, ambas DDP-TRICVALVE).

**Onde o módulo está:** a Fase 2 está completa do lado do Hub — espelho, leitura, cron, e a criação entregue. **O que falta não é código.**

### 1. 🔴 O primeiro teste real da criação — é o único bloqueio

Criar uma RM pela tela, na OP **`2026-0503`**, com **insumo que a OP realmente consome**: a RM **baixa material de verdade** quando o almoxarifado atender. Não é teste de fumaça.

**De preferência com `maria.santos@`**, não com o Pedro. Ela tem `producao.rm.create` e o centro de custo certo; o Pedro vê tudo por bypass de `is_admin` e **erro de permissão não aparece para ele** — é a lição da OP-1.6, e ela é o teste de verdade do gate.

**Conferir depois do próximo sync (≤3h):**

```
select numero, origem, tipo_atendimento, status, codigo_funcionario, codigo_usuario
  from op_reqmat where numero = '<o número criado>';
```

- `origem = 'Importação'` — a métrica de vazamento da §10.7 nasce funcionando;
- `tipo_atendimento = 'Manual'` — **viva, não morta** (o passo 2 pegou);
- `codigo_funcionario` batendo com o perfil de quem criou;
- ⚠ `codigo_usuario` virá da **conta de serviço**, e isso é o esperado (§10.24).

⚠ **Um caminho está codificado e comentado, mas NUNCA rodou:** "Insert OK + Update falho", o estado em que a RM existe no ERP e nasce morta. Exercitá-lo de propósito exigiria tirar `ReqMat/SaveReqMat` da whitelist **entre** os dois passos — intrusivo demais em produção. **A primeira ocorrência real vira a validação**; a tela guia o operador de qualquer forma ("Concluir envio (passo 2)", com "Nova RM" bloqueada naquela OP).

### 2. Quem abre OP — a pergunta segue aberta, e é maior do que parece

Há 2 OPs `ABERTA` (`2026-0501`, `2026-0503`), ambas DDP-TRICVALVE, e **a produção roda sem OP nenhuma**, segundo o Pedro.

⇒ **O módulo não está digitalizando um processo existente: está criando um que não existe.** Isso muda o que "adoção" significa aqui — não é migrar quem já preenche o FRM-07-11, é convencer a fábrica a abrir OP antes de requisitar. Enquanto a resposta não vier, `gestor_producao` continua sem titular (§10.23, decisão aberta nº 1).

### 3. BL-21 — endpoint de lotes disponíveis

Pré-requisito **só do atendimento**. É a última peça faltante do fluxo completo (§10.21), e o protocolo de captura está no próprio BL-21 (Preserve log ligado ANTES de abrir a seleção de lote — escapou de três tentativas).

### 4. Fila de Controller, sem bloquear o módulo

- **BL-15** — fechado na prática pela §10.16, mas a linha permanece no backlog como registro.
- **BL-22** — ⚠ **1 galão de `001.003.00032` não voltou** no estorno (11 → 10): oito dos nove estornaram. **Relevante para a Fase 3**, que depende de o estorno devolver tudo.
- **BL-23** — custo negativo em 41 produtos, com a lista concreta já levantada das 18 RMs de reclamação.
- **As nove perguntas do almoxarifado** (§8.2 do relatório de 31/07) — entre elas **quem realmente atende**, que os dados não dizem (`CodigoFuncionarioAtendente` é sempre `0000165`, padrão do local `001`).

### 5. Dívida deste ciclo, registrada e não paga

**BL-27** (8 das 9 `call_sync_*_cron` abertas a `anon`/`authenticated` — a OP-2.6 provou que revogar não quebra tela nenhuma) e **BL-28** (~137 KB de duplicação nas telas de cron, agora seis). Nenhuma das duas é do módulo OP; ambas são tarefa própria, com regressão.

---

## 11.3 — Ponto de retomada VIGENTE — 08/08/2026

> ⚠ **Esta subseção substitui a §11.2**, mantida como histórico (disciplina da §6.3-N: nada é apagado). **Os dois bloqueios da §11.2 caíram em 08/08/2026:** o primeiro teste real da criação aconteceu (RM `0000002286`, ciclo em 10 s) e o teste com **usuário NÃO-admin** também (RM `0000002287`, por `pscrignoli@gmail.com`) — este último era a pendência aberta desde 06/08, quando se descobriu que o Pedro é o único `is_admin` de 52. O defeito que o primeiro teste revelou virou a OP-2.8, **entregue, aplicada, redeployada, publicada e validada no mesmo dia**.

**Onde o módulo está:** a criação de RM funciona ponta a ponta — do clique na tela ao ERP, e de volta ao espelho no mesmo instante — e a cadeia de permissões está exercitada sem bypass. **A Fase 2 está fechada.** O que resta abaixo **não bloqueia o uso da tela**.

### 1. Quem abre OP — segue aberta, e é a maior das cinco

Há **2 OPs `ABERTA`** (`2026-0501`, `2026-0503`), ambas DDP-TRICVALVE, e **a produção roda sem OP nenhuma**.

⇒ **O módulo não digitaliza um processo existente: cria um que não existe.** Isso muda o que "adoção" significa aqui — não é migrar quem já preenche o FRM-07-11, é convencer a fábrica a abrir OP **antes** de requisitar. Enquanto a resposta não vier, `gestor_producao` continua sem titular (§10.23, decisão aberta nº 1). Nenhuma tarefa de código resolve isto.

### 2. ~~BL-21 · AT-1 · AT-2~~ ✅ **TODOS FEITOS — a Fase 4 não tem mais incógnita técnica**

**BL-21 fechado em 08/08** (endpoint de lotes, §10.30) e **AT-1 + AT-2 fechados em 09/08** (§10.31): whitelist liberada e provada, e o `RelacionarCtrlLoteLocArmaz` revelou-se **o alocador FEFO do servidor** — o Hub não precisa implementar FEFO. ⇒ **Resta a Fase 4 como construção pura: AT-3 (RPC), AT-4 (serviço), AT-5 (tela), AT-6 (registro)**, no `PLANO-RM-ATENDIMENTO.md`. 🔴 Dois requisitos herdados: a **releitura antes de finalizar** (§10.30) e a **guarda de âncora** — `FinalizarAtendimento` responde **200 com objeto vazio** quando o payload não presta (§10.31).

### 3. Fila de Controller, sem bloquear o módulo

- **BL-15** — fechado na prática pela §10.16; a linha permanece no backlog como registro.
- **BL-22** — ⚠ **1 galão de `001.003.00032` não voltou** no estorno (11 → 10): oito dos nove estornaram. **Relevante para a Fase 3**, que depende de o estorno devolver tudo.
- **BL-23** — custo negativo em 41 produtos, com a lista concreta já levantada das 18 RMs de reclamação.
- **As nove perguntas do almoxarifado** (§8.2 do relatório de 31/07) — entre elas **quem realmente atende**, que os dados não dizem (`CodigoFuncionarioAtendente` é sempre `0000165`, padrão do local `001`). ⚠ Ganhou urgência: o papel `almoxarifado` existe, está sem nenhum usuário, e a Fase 4 é para ele.
- **BL-29** — ⚠ **a captura do BL-21 custou um atendimento real.** A RM `0000002283` foi atendida integralmente: **30 unidades de `001.003.00059` baixaram do estoque** (lote `0002467`, saldo 674 → 644). **Verificar com o almoxarifado se o material foi fisicamente entregue à produção.**
- **BL-30** — ⚠ classificação contábil (`ItemReqMatClasseRecdespChildList`, `11.03` a 100%) aparece no item pelo `Relacionar` e **não vinha em nenhum `Load`**. Verificar se as RMs criadas pelo Hub (`2286`, `2287`) nasceram sem ela.

### 4. O caminho "Insert OK + Update falho" continua sem NUNCA ter rodado

Único item do módulo que segue sem prova de campo. Exercitá-lo de propósito exigiria tirar `ReqMat/SaveReqMat` da whitelist **entre** os dois passos — intrusivo demais em produção. A tela o trata: bloco **"Envios incompletos"** (em vermelho), botão "Concluir envio (passo 2)" e "Nova RM" bloqueada naquela OP, para o reflexo do operador não duplicar a RM no ERP. **A primeira ocorrência real vira a validação.**

### 5. Número a vigiar: a fila de produção contra o `LOAD_BATCH`

A RM semeada entra na fila do sync **no fim** — a ordem é `codigo_tipo_req_mat asc` → `detalhes_carregados_em asc`, e a semeadura carimba `now()`. Hoje há folga: fila de produção **123** contra teto **150**. **Acima de 150 ela passa a ser a última relida.** Conferir em `sync_runs` de tempos em tempos.
