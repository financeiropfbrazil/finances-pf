...
| OP-1.2 | RLS + RPCs de escrita | EM ANDAMENTO | 23/07/2026 | **SQL redigido — `sql/OP-1.2.sql` (v2)** (não assume schema: `hub_*` e `op_ordens` lidos ao vivo). v2 acrescenta colunas `fechada_*`, ramo FECHADA, 2 RPCs de aprovação/comunicação, gate em subselect, `op_numeracao` deny-all. Aguarda **Pedro colar no SQL Editor + verificação** → então CONCLUÍDA. |
