...
> ⚠ **Esta subseção substitui a §11.2**, mantida como histórico (disciplina da §6.3-N: nada é apagado). **O bloqueio da §11.2 caiu**: o primeiro teste real da criação aconteceu em 08/08/2026 e o ciclo de três passos passou. O que ele revelou virou a OP-2.8.

**Onde o módulo está:** a criação de RM funciona ponta a ponta no ERP. A OP-2.8 está **entregue no repo e não aplicada** — e é o único item que bloqueia o uso real da tela.

### 1. 🔴 Aplicar a OP-2.8, NESTA ORDEM

A ordem não é preferência: cada passo fora de lugar produz um defeito visível em produção.

1. **`sql/OP-2.9.sql`** no SQL Editor. Conferir na verificação (a) que **`anon = FALSE`** — se vier `true`, o `revoke from anon` não rodou, e é o defeito da OP-2.7 se repetindo.
2. **Redeploy do `sync-reqmat`**, **fora das janelas de cron** (`:00 :10 :15 :25 :30 :45`; este job roda 09h25/12h25/15h25/18h25 BRT). ⚠ Sem ele, a função em produção não conhece a guarda de recência e a primeira RM criada pode aparecer riscada como "Excluída no ERP".
3. **Push + Publish** no Lovable.
4. **Criar uma RM de teste na OP `2026-0503`** e conferir que ela aparece na fila **imediatamente**, com o sinal discreto "criada agora". Depois do primeiro sync, conferir que a linha **não mudou de formato** — é a prova do mapper compartilhado (verificação (f) do `sql/OP-2.9.sql`).
5. **De preferência com `maria.santos@`**, não com o Pedro: ele vê tudo por bypass de `is_admin` e erro de permissão não aparece para ele — a lição da OP-1.6, e a RPC nova tem gate próprio que nunca foi exercitado por um não-admin.

### 2. O caminho "Insert OK + Update falho" segue codificado e NUNCA rodou

Inalterado desde a §11.2: exercitá-lo de propósito exigiria tirar `ReqMat/SaveReqMat` da whitelist **entre** os dois passos — intrusivo demais em produção. **A primeira ocorrência real vira a validação**; a tela guia o operador com "Concluir envio (passo 2)" e "Nova RM" bloqueada naquela OP. Desde a OP-2.8 esse estado tem lugar próprio na tela: o bloco **"Envios incompletos"**, em vermelho.

### 3. Número a vigiar: a fila de produção contra o `LOAD_BATCH`

A RM semeada entra na fila do sync **no fim** (a ordem é `detalhes_carregados_em asc` e a semeadura carimba `now()`). Hoje há folga — fila de produção **123** contra teto **150**. Se a fila de produção passar de 150, a RM criada pelo Hub passa a ser a última a ser relida. Medir em `sync_runs` de tempos em tempos.

### 4. Segue aberto, sem bloquear (herdado da §11.2)

- **Quem abre OP** — a produção roda sem OP nenhuma; o módulo não digitaliza um processo existente, **cria um que não existe**. Enquanto a resposta não vier, `gestor_producao` segue sem titular.
- **BL-21** — endpoint de lotes disponíveis; pré-requisito só do atendimento.
- **BL-22** (1 galão que não voltou no estorno — relevante para a Fase 3), **BL-23** (custo negativo em 41 produtos), **BL-27** e **BL-28**.
- **As nove perguntas do almoxarifado** (§8.2 do relatório de 31/07), entre elas quem realmente atende.
