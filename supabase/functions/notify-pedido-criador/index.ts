...
async function buscarPedido(supabase: any, pedidoId: string): Promise<PedidoData | null> {
  const { data, error } = await supabase
    ...
    .select(
      "id, numero, valor_total, codigo_ind_economico, nome_entidade, cnpj_entidade, proximo_aprovador, numero_req_comp, status_aprovacao, aprovado, criado_no_hub, criado_por_user_id, criado_por_nome",
    )
    .eq("id", pedidoId)
    .single();
  if (error || !data) {
