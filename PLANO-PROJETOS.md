...
| L4 | Frontend: `projetosService.ts` novo + `ProjetoRequisicoes.tsx` chamando RPCs + botão Novo no Actual + erro completo | finances-pf (agente edita+push) | ✅ 07/08 — no `main`, **aguardando Publish do Pedro** |
...
| `sql/PROJ-L4.0-marcar_email_aprovacao.sql` | RPC `projeto_marcar_email_aprovacao` — corrige o **A-7** (D-15) | ⬜ pendente — **rodar antes do Publish do L4** |
