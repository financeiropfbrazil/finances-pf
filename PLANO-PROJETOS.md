...
| D-17 | A conta `nfe@pfbrazil.com` recebe `alvo_usuario`? | **NÃO — APROVADO em 07/08.** É conta **funcional** (NF-e), não pessoa, e não tem login no Alvo. Fica **sem** `alvo_usuario`, e o bloqueio no envio (b4) é o comportamento **correto e desejado** para ela — não é bug a ser contornado. Se um dia precisar enviar pedido, **cria-se primeiro um login no Alvo**; nunca emprestar o de outra pessoa. Login confirmado e liberado: `ana.sanches@pfbrazil.com` → **`ANA.SANCHES`** (verificado no Alvo pelo Pedro, não deduzido). |
| D-16 | Manter a validação de teto no **front**? | **SIM — APROVADO em 07/08.** A tela continua barrando antes de chamar a RPC. **Consequência assumida:** `teto_rejeitado` deixa de ser métrica de rotina e vira **alarme de anomalia** — só grava se alguém contornar a tela e chamar a RPC direto. A trava do banco (D-1) permanece como rede de segurança. **O L5 NÃO inclui "tentativas de estouro" como métrica** — sai da `v_projeto_resumo` e não vira agregação em lugar nenhum. |
| D-15 | Como cobrir o **A-7** (`email_aprovacao_enviado_em` não grava para não-admin)? | **PROPOSTA — aguarda aval.** RPC pequena `projeto_marcar_email_aprovacao(p_projeto_id uuid)`, `SECURITY DEFINER`, gate = admin **ou** responsável, **sem restrição de fase** (ela roda depois que a fase já virou `budget_em_aprovacao` — é essa a razão da falha). Entregaria como `sql/PROJ-L4.0-marcar_email_aprovacao.sql` junto do L4. Alternativa descartada: gravar o timestamp dentro de `enviar_budget_para_aprovacao`, que roda **antes** do e-mail e registraria envio que pode nunca ter acontecido. |
...
**R-3 (achado de 07/08 — corrige uma premissa do item L5.3).** O item L5.3 diz que "views herdam das tabelas (security_invoker default)". **Isso está errado.** No PostgreSQL o default é `security_invoker = off`: a view roda com os privilégios do **OWNER** (aqui, `postgres`) e **bypassa o RLS** das tabelas base. Medido neste banco: as `rh_vw_*` têm `{security_invoker=on}`; as de intercompany/compras têm `reloptions` **null** — ou seja, rodam como owner. Sem o flag, `v_projeto_resumo` e `v_projeto_eventos_report` exporiam **todos** os projetos a qualquer `authenticated`, jogando fora o escopo que o L3 acabou de montar. **As duas views DEVEM ser criadas com `WITH (security_invoker = on)`** (PG 17.6 aqui, suportado desde o 15).
...
| P-6 | **`Projetos.tsx` edita projeto via `.upsert()`** — mesma classe do bug 42501: o upsert avalia a WITH CHECK do **INSERT** de `projetos`, que exige `criado_por = auth.uid()`. Enquanto responsável = criador, funciona; no dia em que divergirem, a edição de projeto quebra para o responsável. **⚠️ Nota do Pedro (07/08): depois do L3, `projetos` fica com DOIS critérios de titularidade convivendo — SELECT/UPDATE/DELETE por `responsavel_id`, INSERT por `criado_por`.** Não bloqueia nada hoje; é exatamente o ponto que estoura no primeiro projeto com responsável ≠ criador | **ABERTA** — resolvida de vez pelo P-1 (CRUD de projetos em RPC) | nada hoje |
...
### A-9 — o modal de cadastro de usuário está incompleto (achado de 07/08)

O modal "Editar Usuário" (`settings/Users`) gerencia **só** o `funcionario_alvo_codigo`, com o texto *"Vincular a um funcionário do Alvo é necessário para criar Requisições de Compra"*. **Não expõe o `alvo_usuario`.**

**Consequência:** o admin preenche o funcionário, acha que terminou o cadastro, e a pessoa fica sem conseguir **enviar pedido** — sem nenhuma pista do porquê. Foi exatamente o que aconteceu com a `ana.sanches` e com o `nfe@`. Hoje o `alvo_usuario` só se preenche por `UPDATE` manual no SQL Editor (foi assim com Elisangela, Mirlene e Ryan, conforme o PLANO-PEDIDOS) — ou seja, um campo que decide identidade no ERP vive fora da tela de cadastro.

**Correção:** acrescentar o campo **"Login no ERP Alvo"** (`alvo_usuario`) ao mesmo modal, com texto explicando que é necessário para **enviar pedidos** ao ERP — espelhando o texto que já existe para funcionário/requisições. Cadastro completo num lugar só.
